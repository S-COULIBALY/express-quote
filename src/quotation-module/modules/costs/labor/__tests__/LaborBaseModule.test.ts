...
describe('LaborBaseModule', () => {
  ...
  describe('apply method', () => {
    it('should calculate base labor cost using workersCount from context', () => {
      ...
      const expectedCost = laborConfig.BASE_HOURLY_RATE * laborConfig.BASE_WORK_HOURS * expectedWorkers;
      
      const laborCost = result.computed?.costs?.find(c => c.moduleId === 'labor-base');
      expect(laborCost).toBeDefined();
      expect(laborCost?.category).toBe('LABOR');
      expect(laborCost?.amount).toBe(expectedCost);
      expect(laborCost?.metadata?.hourlyRate).toBe(laborConfig.BASE_HOURLY_RATE);
      expect(laborCost?.metadata?.estimatedHours).toBe(laborConfig.BASE_WORK_HOURS);
    ...
    it('should always use 7 hours (one work day)', () => {
      ...
      const result = module.apply(smallVolumeContext);
      
      const laborCost = result.computed?.costs?.find(c => c.moduleId === 'labor-base');
      expect(laborCost?.metadata?.estimatedHours).toBe(laborConfig.BASE_WORK_HOURS); // 7h
    });
    ...
    it('should not adjust hours based on access complexity (simplified logic)', () => {
      ...
      const result = module.apply(complexContext);
    ...
    it('should not apply if workersCount is not available (WorkersCalculationModule not executed)', () => {
      ...
      const result = module.apply(noWorkersContext);
