...
describe("ManualHandlingRiskCostModule", () => {
  let mod: ManualHandlingRiskCostModule;
  ...
  beforeEach(() => {
    mod = new ManualHandlingRiskCostModule();
    baseContext = {
      serviceType: "MOVING",
      region: "IDF",
  ...
  describe("apply", () => {
    it("should add risk cost if lift refused (pickup only)", () => {
      ...
      const riskCost = result.computed?.costs.find(
        (c) => c.moduleId === "manual-handling-risk-cost",
      );
