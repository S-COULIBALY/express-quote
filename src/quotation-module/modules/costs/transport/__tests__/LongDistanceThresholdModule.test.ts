...
describe('LongDistanceThresholdModule', () => {
  let module: LongDistanceThresholdModule;
  let baseContext: QuoteContext;

  beforeEach(() => {
    module = new LongDistanceThresholdModule();
    baseContext = {
      serviceType: 'MOVING',
      region: 'IDF',
  ...
  describe('isApplicable', () => {
    it('should be applicable if distance is calculated', () => {
      baseContext.computed!.distanceKm = 30;
      expect(module.isApplicable(baseContext)).toBe(true);
    });

    it('should not be applicable if distance is not calculated', () => {
  ...
  describe('apply', () => {
    it('should detect long distance if distance > 50 km', () => {
      ...
      expect(result.computed?.activatedModules).toContain('long-distance-threshold');
    });
    ...
    it('should not apply if distance is not calculated', () => {
      ...
      expect(result.computed?.activatedModules).not.toContain('long-distance-threshold');
    });
  });
});
