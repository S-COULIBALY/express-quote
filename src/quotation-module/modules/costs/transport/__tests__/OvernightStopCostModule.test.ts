...
describe('OvernightStopCostModule', () => {
  let module: OvernightStopCostModule;
  ...
  beforeEach(() => {
    module = new OvernightStopCostModule();
    baseContext = {
      serviceType: 'MOVING',
      region: 'IDF',
  ...
  describe('apply', () => {
    describe('conditions d\'application', () => {
      it('should not apply if distance is below threshold', () => {
        ...
        const cost = result.computed?.costs.find(c => c.moduleId === 'overnight-stop-cost');
      ...
      it('should apply if distance > threshold AND forceOvernightStop is true', () => {
        ...
        expect(cost?.category).toBe('TRANSPORT');
        expect(result.computed?.activatedModules).toContain('overnight-stop-cost');
      });
    });
    ...
    describe('calcul des coûts', () => {
      it('should calculate correct cost for 2 workers', () => {
        ...
        const expectedTotal = expectedHotelCost + config.SECURE_PARKING_COST + expectedMealCost;
        
        expect(cost?.amount).toBeCloseTo(expectedTotal, 2);
        expect(cost?.amount).toBe(350.00); // (2 × 120) + 50 + (2 × 30) = 240 + 50 + 60 = 350
      });
      ...
      it('should calculate correct cost for 3 workers', () => {
        ...
        expect(cost?.amount).toBeCloseTo(expectedTotal, 2);
        expect(cost?.amount).toBe(470.00); // (3 × 120) + 50 + (3 × 30) = 360 + 50 + 90 = 470
      });
      ...
      it('should use default 2 workers if workersCount is not set', () => {
        ...
        expect(cost?.amount).toBeCloseTo(expectedTotal, 2);
        expect(cost?.amount).toBe(350.00);
      ...
      it('should calculate correct cost for 4 workers', () => {
        ...
        expect(cost?.amount).toBeCloseTo(expectedTotal, 2);
        expect(cost?.amount).toBe(650.00); // (4 × 120) + 50 + (4 × 30) = 480 + 50 + 120 = 650
      });
    });
    ...
    describe('métadonnées', () => {
      ...
      it('should add requirement for overnight stop', () => {
        ...
        const requirement = result.computed?.requirements.find(
          r => r.type === 'OVERNIGHT_STOP_REQUIRED' && r.moduleId === 'overnight-stop-cost'
        );
      ...
      it('should add operational flag', () => {
        ...
        expect(result.computed?.operationalFlags).toContain('OVERNIGHT_STOP_REQUIRED');
      });
      ...
      it('should update computed metadata', () => {
        ...
        expect(result.computed?.metadata.overnightStopCost).toBe(350.00);
      });
    });
    ...
    describe('label du coût', () => {
      it('should include workers count in label', () => {
        ...
        const cost = result.computed?.costs.find(c => c.moduleId === 'overnight-stop-cost');
        expect(cost?.label).toBe('Arrêt nuit équipe (3 pers. + parking sécurisé)');
      });
    });
  });
});
