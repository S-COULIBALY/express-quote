...
describe("DistanceModule", () => {
  let mod: DistanceModule;
  const distanceConfig = MODULES_CONFIG.distance;

  beforeEach(() => {
    mod = new DistanceModule();
  });
  ...
  it("should use distance from context when provided", () => {
    ...
    const expectedTravelTime = Math.round(
      3.2 * distanceConfig.TRAVEL_TIME_FACTOR,
    );

    expect(result.computed?.distanceKm).toBe(3.2);
    expect(result.computed?.estimatedTravelTimeMinutes).toBe(
      expectedTravelTime,
    );
    ...
    expect(result.computed?.metadata?.maxDistanceThreshold).toBe(
      distanceConfig.MAX_DISTANCE_KM,
    );
    expect(result.computed?.metadata?.longDistanceThreshold).toBe(
      distanceConfig.LONG_DISTANCE_THRESHOLD_KM,
    );
    expect(result.computed?.metadata?.travelTimeFactor).toBe(
      distanceConfig.TRAVEL_TIME_FACTOR,
    );
  });
  ...
  it("should use default distance when not provided", () => {
    ...
    const result = mod.apply(ctx);
    const expectedTravelTime = Math.round(
      distanceConfig.DEFAULT_DISTANCE_KM * distanceConfig.TRAVEL_TIME_FACTOR,
    );

    expect(result.computed?.distanceKm).toBe(
      distanceConfig.DEFAULT_DISTANCE_KM,
    );
    expect(result.computed?.estimatedTravelTimeMinutes).toBe(
      expectedTravelTime,
    );
    ...
    expect(result.computed?.metadata?.distanceValidated).toBe(
      distanceConfig.DEFAULT_DISTANCE_KM,
    );
  });
  ...
  it("should have correct module metadata", () => {
    expect(mod.id).toBe("distance-calculation");
    expect(mod.description).toBe(
      "Récupère la distance calculée par le formulaire",
    );
    expect(mod.priority).toBe(30);
  });
});
