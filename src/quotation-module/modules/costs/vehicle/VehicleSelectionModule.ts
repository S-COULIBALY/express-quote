...
export class VehicleSelectionModule implements QuoteModule {
  ...
  apply(ctx: QuoteContext): QuoteContext {
        ...
        computed: {
          ...
          costs: [
            ...computed.costs,
            {
              ...
              category: "VEHICLE",
              metadata: {
                primaryVehicle: defaultVehicleType,
    ...
    const vehicleSelection =
      this.calculateOptimalVehicleSelection(adjustedVolume);
    ...
    // Véhicule principal
    console.log(
      `         Véhicule principal: ${vehicleSelection.primary.type} (${vehicleSelection.primary.capacity} m³)`,
    );
    console.log(
      `            Volume utilisé: ${vehicleSelection.primary.volumeUsed.toFixed(2)} m³`,
    );
    ...
    if (vehicleSelection.secondary) {
      console.log(
        `         Véhicule complémentaire: ${vehicleSelection.secondary.type} (${vehicleSelection.secondary.capacity} m³)`,
      );
      console.log(
        `            Volume restant: ${vehicleSelection.secondary.volumeUsed.toFixed(2)} m³`,
      );
      console.log(
        `            Capacité la plus proche: ${vehicleSelection.secondary.capacity} m³`,
      );
      console.log(`            Coût: ${vehicleSelection.secondary.cost}€`);
    }

    console.log(
      `      Calcul: ${vehicleSelection.primary.cost}€${vehicleSelection.secondary ? ` + ${vehicleSelection.secondary.cost}€` : ""} = ${totalVehicleCost.toFixed(2)}€`,
    );
        ...
        costs: [
          ...computed.costs,
          {
            ...
            metadata: {
              ...
              secondaryVehicleCapacity:
                vehicleSelection.secondary?.capacity || null,
              secondaryVolumeUsed:
                vehicleSelection.secondary?.volumeUsed || null,
  ...
  private calculateOptimalVehicleSelection(adjustedVolume: number): {
    ...
    let secondary:
      | { type: string; capacity: number; cost: number; volumeUsed: number }
      | undefined;
  ...
  ): "CAMION_12M3" | "CAMION_20M3" | "CAMION_30M3" {
   ...
   */
  private findBestVehicleForVolume(
    volume: number,
  ): "CAMION_12M3" | "CAMION_20M3" | "CAMION_30M3" {
    const vehicleConfig = MODULES_CONFIG.vehicle;
    ...
    const sufficientVehicles: Array<{
      type: "CAMION_12M3" | "CAMION_20M3" | "CAMION_30M3";
      capacity: number;
      distance: number;
    }> = [];
