...
describe('StorageCostModule', () => {
  const module = new StorageCostModule();
  const config = MODULES_CONFIG.crossSelling;

  it('devrait avoir les bonnes propriétés', () => {
    expect(module.id).toBe('storage-cost');
    expect(module.description).toBe('Calcule le coût du stockage temporaire si accepté');
    expect(module.priority).toBe(87);
    expect(module.dependencies).toEqual(['storage-requirement']);
  });
  ...
  it('devrait ajouter le coût de stockage', () => {
    ...
    const cost = result.computed?.costs.find(c => c.moduleId === 'storage-cost');
    ...
    const expectedCost = 30 * config.STORAGE_COST_PER_M3_PER_MONTH * expectedMonths; // 1800€
