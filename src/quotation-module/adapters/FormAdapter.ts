...
import {
  modalSelectionsToQuoteContext,
  ModalSelections,
} from "./ModalSelectionsAdapter";
...
export interface FormData {
  ...
  scheduledDate?: string | Date; // Alias depuis prepareRequestData
  flexibility?: "NONE" | "PLUS_MINUS_3" | "PLUS_MINUS_7" | string;
  flexibilite?: string; // Alias français depuis le formulaire
  ...
  distanceEstimee?: number | string; // Alias depuis prepareRequestData
...
export class FormAdapter {
  ...
  static toQuoteContext(formData: FormData): QuoteContext {
    ...
    const context: QuoteContext = {
      ...
      // Date (supporte dateSouhaitee et scheduledDate comme alias)
      movingDate: this.normalizeDate(
        formData.movingDate || formData.dateSouhaitee || formData.scheduledDate,
      ),
      flexibility:
        this.convertFlexibilityFromForm(
          formData.flexibility || formData.flexibilite,
        ) ?? this.normalizeFlexibility(formData.flexibility),

      // Volume (calculateur V3 côté client uniquement)
      volumeMethod: "FORM",
      estimatedVolume: this.normalizeNumber(formData.estimatedVolume),
      volumeConfidence: this.calculateVolumeConfidence(
        this.normalizeNumber(formData.estimatedVolume),
      ),
      ...
      departureAddress:
        ...
        "",
      departurePostalCode: this.normalizePostalCode(
        formData.departurePostalCode ?? formData.pickupPostalCode,
      ),
      departureCity: this.normalizeString(
        formData.departureCity ?? formData.pickupCity,
      ),
      pickupFloor: this.normalizeInteger(formData.pickupFloor),
      pickupHasElevator:
        this.convertElevatorEnumToBoolean(formData.pickupElevator) ??
        this.normalizeBoolean(formData.pickupHasElevator),
      pickupElevatorSize:
        this.convertElevatorEnumToSize(formData.pickupElevator) ??
        this.normalizeElevatorSize(formData.pickupElevatorSize),
      ...
      pickupParkingAuthorizationRequired: this.normalizeBoolean(
        formData.pickupParkingAuthorizationRequired,
      ),
      pickupSyndicTimeSlot: this.normalizeBoolean(
        formData.pickupSyndicTimeSlot,
      ),
      ...
      arrivalAddress:
        ...
        "",
      arrivalPostalCode: this.normalizePostalCode(
        formData.arrivalPostalCode ?? formData.deliveryPostalCode,
      ),
      arrivalCity: this.normalizeString(
        formData.arrivalCity ?? formData.deliveryCity,
      ),
      deliveryFloor: this.normalizeInteger(formData.deliveryFloor),
      deliveryHasElevator:
        this.convertElevatorEnumToBoolean(formData.deliveryElevator) ??
        this.normalizeBoolean(formData.deliveryHasElevator),
      deliveryElevatorSize:
        this.convertElevatorEnumToSize(formData.deliveryElevator) ??
        this.normalizeElevatorSize(formData.deliveryElevatorSize),
      // Distance de portage : utilise valeur fournie OU estimation automatique
      ...
      deliveryStreetNarrow: this.normalizeBoolean(
        formData.deliveryStreetNarrow,
      ),
      deliveryParkingAuthorizationRequired: this.normalizeBoolean(
        formData.deliveryParkingAuthorizationRequired,
      ),
      deliverySyndicTimeSlot: this.normalizeBoolean(
        formData.deliverySyndicTimeSlot,
      ),

      // Distance (supporte distanceEstimee comme alias)
      distance: this.normalizeNumber(
        formData.distance ?? formData.distanceEstimee,
      ),

      // Inventaire - Priorité aux modals, fallback sur champs directs
      bulkyFurniture:
        modalMappedContext.bulkyFurniture ??
        this.normalizeBoolean(formData.bulkyFurniture),
      ...
      artwork:
        modalMappedContext.artwork ?? this.normalizeBoolean(formData.artwork),
      builtInAppliances: this.normalizeBoolean(formData.builtInAppliances), // Pas dans les modals actuellement

      // Logistique
      multiplePickupPoints: this.normalizeBoolean(
        formData.multiplePickupPoints,
      ),
      // Stockage temporaire - Priorité aux modals, fallback sur champ direct
      temporaryStorage:
        modalMappedContext.temporaryStorage ??
        this.normalizeBoolean(formData.temporaryStorage),
      ...
      packing:
        modalMappedContext.packing ?? this.normalizeBoolean(formData.packing),
      unpacking:
        modalMappedContext.unpacking ??
        this.normalizeBoolean(formData.unpacking),
      cleaningEnd:
        modalMappedContext.cleaningEnd ??
        this.normalizeBoolean(formData.cleaningEnd),
      ...
      deliveryFurnitureLift: this.normalizeBoolean(
        formData.deliveryFurnitureLift,
      ),

      // Juridique
      declaredValue: this.normalizeNumber(formData.declaredValue),
      refuseLiftDespiteRecommendation: this.normalizeBoolean(
        formData.refuseLiftDespiteRecommendation,
      ),

      // Cross-selling fournitures (depuis catalogue)
      crossSellingSuppliesTotal: this.normalizeNumber(
        formData.crossSellingSuppliesTotal,
      ),
      // Normaliser les détails des fournitures : transformer 'total' en 'totalPrice' si nécessaire
      crossSellingSuppliesDetails: formData.crossSellingSuppliesDetails?.map(
        (item) => ({
          id: item.id || "",
          name: item.name || "",
      ...
      crossSellingServicesTotal: this.normalizeNumber(
        formData.crossSellingServicesTotal,
      ),
      crossSellingGrandTotal: this.normalizeNumber(
        formData.crossSellingGrandTotal,
      ),
    };
   ...
   */
  private static convertElevatorEnumToBoolean(
    elevator?: string,
  ): boolean | undefined {
   ...
   * - 'large' → 'LARGE'
   */
  private static convertElevatorEnumToSize(
    elevator?: string,
  ): "SMALL" | "STANDARD" | "LARGE" | undefined {
    if (!elevator || typeof elevator !== "string") return undefined;
   ...
   */
  private static convertFlexibilityFromForm(
    flexibility?: string,
  ): "NONE" | "PLUS_MINUS_3" | "PLUS_MINUS_7" | undefined {
   ...
   */
  private static calculateVolumeConfidence(
