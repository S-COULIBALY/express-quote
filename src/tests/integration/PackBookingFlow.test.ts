/**
 * Test d'intégration du flux complet de réservation de pack
 * 
 * Ce test couvre toutes les étapes du parcours client pour une réservation
 * de type pack, de la création jusqu'à la confirmation.
 */
...
import { packBookingData } from '../mocks/TestData';
...
describe('Flux de réservation de pack', () => {
  ...
  beforeEach(() => {
    ...
    stripeSessionId = TEST_IDS.PACK.STRIPE_SESSION_ID;
  });

  it('devrait compléter tout le flux de réservation de pack avec succès', async () => {
    ...
    expect(createData).toHaveProperty('type', BookingType.PACK);
    ...
    expect(getData.customer).toHaveProperty('email', packBookingData.customer.email);
