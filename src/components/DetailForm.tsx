...
const DetailFormComponent: React.FC<DetailFormProps> = ({ catalogData }) => {
  console.log(
    "🎯 [ÉTAPE 4] DetailForm - Initialisation du formulaire personnalisé",
  );
  ...
  React.useEffect(() => {
    ...
    console.log(
      "🔢 [SURVEILLANCE] Nombre de champs remplis:",
      Object.keys(formData).length,
    );
    console.log(
      "📝 [SURVEILLANCE] Champs non-vides:",
  ...
  const presetSnapshot = useMemo(
    () => transformedData?.__presetSnapshot,
    [transformedData?.__presetSnapshot],
  );

  // Hook de calcul de prix en temps réel (remplace le calculateur factice)
  const rawPriceCalculator = useRealTimePricing(
    serviceType,
    basePrice,
    presetSnapshot,
  );
  ...
  const movingSubmissionHook = useCatalogueMovingItemSubmission(
    stableTransformedData as any,
    stableCalculatedPrice,
    stableDistance,
  );
  const cleaningSubmissionHook = useCatalogueCleaningItemSubmission(
    stableTransformedData as any,
    stableCalculatedPrice,
  );
  const deliverySubmissionHook = useCatalogueDeliveryItemSubmission(
    stableTransformedData as any,
    stableCalculatedPrice,
    stableDistance,
  );
  ...
  const demenagementSurMesureSubmissionHook =
    useDemenagementSurMesureSubmission(
      stableTransformedData as any,
      stableCalculatedPrice,
      stableDistance,
    );
  const menageSurMesureSubmissionHook = useMenageSurMesureSubmission(
    stableTransformedData as any,
    stableCalculatedPrice,
  );
  ...
  const priceCalculator = useMemo(
    () => ({
      calculatePrice:
        rawPriceCalculator?.calculatePrice || (async () => basePrice),
  ...
  const submissionHook = useMemo(() => {
    ...
    presetType,
    movingSubmissionHook,
    cleaningSubmissionHook,
    deliverySubmissionHook,
    demenagementSurMesureSubmissionHook,
    menageSurMesureSubmissionHook,
  ]);
  ...
  const handleFormDataChange = useCallback(
    async (fieldName: string, value: any, currentFormData: any) => {
      console.log(
        "🔗 [ÉTAPE 10.1] DetailForm - Synchronisation depuis FormGenerator",
      );
      ...
      const hasEnoughData =
        presetType === "catalogueMovingItem-service"
          ? !!(
              enrichedFormData.scheduledDate &&
              enrichedFormData.pickupAddress &&
              enrichedFormData.deliveryAddress
            )
      ...
      if (hasEnoughData) {
        ...
        console.log(
          "⏸️ [ÉTAPE 10.3] Calcul de prix non déclenché (données insuffisantes)",
        );
      }
    },
  ...
  const handlePriceCalculated = useCallback(
    async (price: number, details: any) => {
    ...
    [
      ...
      catalogData.catalogSelection.category,
      rawPriceCalculator.calculatedPrice,
      basePrice,
    ],
  );
  ...
  const handleSubmitSuccess = useCallback(
    ...
    [catalogData.catalogSelection.id, presetType],
  );
  ...
  const handleChange = useCallback(
    (fieldName: string, value: any, currentFormData: any) => {
  ...
  const enrichedConfig = useMemo(() => {
    ...
    const catalogDefaults = {
      ...
      scheduledDate: transformedData.scheduledDate
        ? transformedData.scheduledDate.toISOString().split("T")[0]
        : "",
    ...
    console.log(
      "🎯 [ÉTAPE 6] Génération des valeurs par défaut (catalogDefaults)",
    );
    ...
    const additionalLayoutConfig = {
      ...
      serviceInfo: {
        ...
        description:
          transformedData?.description || catalogData.item.description,
        originalPrice:
          catalogData.catalogSelection.originalPrice ||
          transformedData?.originalPrice,
      },
    };
    ...
    console.log(
      "⚙️ [ÉTAPE 7] Configuration du formulaire - Sélection du preset",
    );
    console.log(
      "➡️ [FLUX] Passage à FormGenerator avec configuration enrichie",
    );
    ...
    switch (presetType) {
      case "demenagement-sur-mesure": {
        const presetConfig = getDemenagementSurMesureServiceConfig({
          ...
          onPriceCalculated: (price: number, details: any) =>
            handlePriceCalculated(price, details),
    ...
    <div className="max-w-7xl mx-auto px-2 sm:px-6 lg:px-8 py-1 sm:py-8">
      ...
      <div className="text-center mb-1 sm:mb-8">
        ...
        <p className="text-sm sm:text-base text-gray-600">
          Configurez les détails de votre{" "}
          {catalogData.catalogSelection.marketingTitle || catalogData.item.name}{" "}
          selon vos besoins
        </p>
      </div>
...
export const DetailForm = memo(DetailFormComponent, (prevProps, nextProps) => {
  const shouldSkipRender =
    prevProps.catalogData.catalogSelection.id ===
      nextProps.catalogData.catalogSelection.id &&
