...
import {
  ...
  ExternalProvider,
  EmailTypeConfig,
} from "../../domain/entities/EmailConfig";
...
export class PrismaEmailConfigRepository implements IEmailConfigRepository {
  ...
  public async getEmailConfig(): Promise<EmailConfig> {
    try {
      ...
      const configMap = Array.isArray(emailConfigs)
        ? emailConfigs.reduce(
      ...
      return new EmailConfig(
        ...
        Array.isArray(externalProviders)
  ...
  public async updateEmailConfig(config: EmailConfig): Promise<EmailConfig> {
    try {
      ...
      await this.upsertConfig(
        ...
        config.professionalsEmails,
      );
      await this.upsertConfig(
        "recipients.notifications",
        config.notificationsEmails,
      );
  ...
  public async getExternalProviders(): Promise<ExternalProvider[]> {
    try {
      ...
      console.error(
        "Erreur lors de la récupération des prestataires externes:",
        error,
      );
      return [];
    }
  ...
  public async addExternalProvider(
    provider: Omit<ExternalProvider, "id">,
  ...
  public async updateExternalProvider(
    ...
    try {
      ...
      console.error(
        "Erreur lors de la mise à jour du prestataire externe:",
        error,
      );
      throw error;
    }
  ...
  public async deleteExternalProvider(id: string): Promise<void> {
    try {
      ...
      console.error(
        "Erreur lors de la suppression du prestataire externe:",
        error,
      );
      throw error;
    }
