...
import { ConfigurationCategory, PricingConfigKey, BusinessTypePricingConfigKey } from "../../domain/configuration/ConfigurationKey";
...
export class MovingQuoteStrategy implements QuoteStrategy {
  ...
  private async initializeRules(): Promise<void> {
    try {
      const businessRules = await this.unifiedDataService.getBusinessRulesForEngine(UnifiedServiceType.MOVING);
      if (businessRules.length > 0) {
        console.log(`✅ [MOVING-STRATEGY] ${businessRules.length} règles métier chargées depuis UnifiedDataService`);
        // Remplacer le RuleEngine avec les nouvelles règles
        this.ruleEngine = new RuleEngine(businessRules);
      } else {
        console.log('⚠️ [MOVING-STRATEGY] Aucune règle métier trouvée, utilisation des règles par défaut');
      }
    } catch (error) {
      console.warn('⚠️ [MOVING-STRATEGY] Erreur lors du chargement des règles métier:', error);
  ...
  canHandle(serviceType: string): boolean {
    return serviceType === ServiceType.MOVING ||
  ...
  async calculate(context: QuoteContext): Promise<Quote> {
    ...
    try {
      ...
      if (this.isPackingUnchanged(context) && (data.defaultPrice || data.calculatedPrice || data.totalPrice)) {
        ...
        console.log(`🎯 [MOVING-STRATEGY] PACKING inchangé détecté - Prix par défaut SANS promotions: ${defaultPrice}€`);
        ...
        const defaultQuote = new Quote(
          ...
          [],
          serviceType,
          [{ label: "Prix par défaut (PACKING non modifié)", amount: defaultPrice }]
        );
      ...
      console.log('🎯 ═══════════════════════════════════════════════════════════════');
      console.log('💰 Prix de base:', finalQuote.getBasePrice().getAmount(), '€');
      console.log('💰 Prix final:', finalQuote.getTotalPrice().getAmount(), '€');
      console.log('📋 Nombre de règles appliquées:', finalQuote.getDiscounts().length);
      ...
      calculationDebugLogger.logCalculationError(error, 'MOVING_STRATEGY', data);
      throw error;
    }
  }
  ...
  private async calculateBasePriceOnly(context: QuoteContext): Promise<{ baseTotal: number; details: { label: string; amount: number }[] }> {
    ...
    const baseRate = pricingConstants['MOVING_BASE_PRICE_PER_M3'] || DefaultValues.MOVING_BASE_PRICE_PER_M3;
    const laborRate = pricingConstants['SERVICE_WORKER_PRICE_PER_HOUR'] || DefaultValues.WORKER_HOUR_RATE;
    const truckRate = pricingConstants['MOVING_TRUCK_PRICE'] || DefaultValues.VEHICLE_FLAT_FEE;
    const distanceRate = pricingConstants['MOVING_DISTANCE_PRICE_PER_KM'] || DefaultValues.UNIT_PRICE_PER_KM;
    const fuelRate = pricingConstants['FUEL_PRICE_PER_LITER'] || DefaultValues.FUEL_PRICE_PER_LITER;
    const tollRate = pricingConstants['TOLL_COST_PER_KM'] || DefaultValues.TOLL_COST_PER_KM;
    ...
    const freeDistanceKm = pricingConstants['MOVING_FREE_DISTANCE_KM'] || DefaultValues.INCLUDED_DISTANCE;
    ...
    if (serviceType === ServiceType.MOVING) {
      ...
      if (!volume || volume === 0) {
        throw new Error('MOVING (déménagement sur mesure) requiert un volume non nul');
      }

      // ✅ CORRECTION: Le camion se loue par jour (7h = 1 jour par défaut)
      const hoursPerDay = pricingConstants['HOURS_PER_DAY'] || 7;
      const numberOfDays = Math.ceil(duration / hoursPerDay);

      // 🧮 Nombre de déménageurs recommandé (INFORMATIF UNIQUEMENT)
      const workersPerM3Threshold = pricingConstants['MOVING_WORKERS_PER_M3_THRESHOLD'] || 5;
      const recommendedWorkers = Math.max(1, Math.ceil(volume / workersPerM3Threshold));
      ...
      details = [
        ...
        { label: `Camion (${numberOfDays} jour${numberOfDays > 1 ? 's' : ''})`, amount: truckCost },
        { label: `Distance (au-delà de ${freeDistanceKm} km)`, amount: distanceCost },
        { label: `Carburant (au-delà de ${freeDistanceKm} km)`, amount: fuelCost },
        { label: `Péages (au-delà de ${freeDistanceKm} km)`, amount: tollCost },
        { label: "Nb déménageurs recommandés (info)", amount: recommendedWorkers },
      ];

      console.log(`🏠 [MOVING-STRATEGY] CALCUL MOVING SUR MESURE (PRIX DE BASE - VOLUME UNIQUEMENT):`);
      console.log(`   └─ Volume: ${volume}m³ × ${baseRate}€ = ${volumeCost.toFixed(2)}€`);
      console.log(`   └─ Déménageurs recommandés (info): ${recommendedWorkers} (calculé: ${volume}m³ ÷ ${workersPerM3Threshold}m³/worker)`);
      console.log(`   └─ Camion: ${truckRate}€ × ${numberOfDays} jour${numberOfDays > 1 ? 's' : ''} = ${truckCost.toFixed(2)}€`);
      console.log(`   └─ Distance: ${chargeableKm}km × ${distanceRate}€ = ${distanceCost.toFixed(2)}€`);
      console.log(`   └─ Carburant: ${chargeableKm}km × ${fuelRate}€ = ${fuelCost.toFixed(2)}€`);
      console.log(`   └─ Péages: ${chargeableKm}km × ${tollRate}€ = ${tollCost.toFixed(2)}€`);
    ...
    // ✅ Cas 2 : PACKING catalogue (MAIN D'ŒUVRE UNIQUEMENT + transport)
      ...
      if (!workers || workers === 0 || !duration || duration === 0) {
        throw new Error('PACKING (pack catalogue) requiert workers et duration non nuls');
      }
      ...
      console.log(`📦 [MOVING-STRATEGY] CALCUL PACKING CATALOGUE (PRIX DE BASE - MAIN D'ŒUVRE UNIQUEMENT):`);
      console.log(`   └─ Main d'œuvre: ${workers} × ${duration}h × ${laborRate}€ = ${laborCost.toFixed(2)}€`);
      console.log(`   └─ Camion: ${truckRate}€ × ${numberOfDays} jour${numberOfDays > 1 ? 's' : ''} = ${truckCost.toFixed(2)}€`);
      console.log(`   └─ Distance: ${chargeableKm}km × ${distanceRate}€ = ${distanceCost.toFixed(2)}€`);
      console.log(`   └─ Carburant: ${chargeableKm}km × ${fuelRate}€ = ${fuelCost.toFixed(2)}€`);
      console.log(`   └─ Péages: ${chargeableKm}km × ${tollRate}€ = ${tollCost.toFixed(2)}€`);
    ...
    // ✅ Cas 3 : MOVING-PREMIUM (volume + cartons + fournitures + taux horaire premium, workers auto-calculés à titre informatif)
      ...
      const boxesPerM3 = pricingConstants['MOVING_BOXES_PER_M3'] || 10;
      const boxPrice = pricingConstants['MOVING_BOX_PRICE'] || 2;
      ...
      const suppliesMultiplier = pricingConstants['MOVING_PREMIUM_SUPPLIES_MULTIPLIER'] || 2.5;
      ...
      const premiumLaborRate = pricingConstants['MOVING_PREMIUM_WORKER_PRICE_PER_HOUR'] || 40;
      ...
      details = [
        ...
        { label: "Fournitures premium (emballage, protection)", amount: suppliesCost },
        { label: `Main d'œuvre premium (${workers} déménageurs × ${duration}h × ${premiumLaborRate}€)`, amount: laborCost },
        { label: `Camion (${numberOfDays} jour${numberOfDays > 1 ? 's' : ''})`, amount: truckCost },
        { label: `Distance (au-delà de ${freeDistanceKm} km)`, amount: distanceCost },
        { label: `Carburant (au-delà de ${freeDistanceKm} km)`, amount: fuelCost },
        { label: `Péages (au-delà de ${freeDistanceKm} km)`, amount: tollCost },
      ...
      console.log(`🏠 [MOVING-STRATEGY] CALCUL MOVING_PREMIUM (PRIX DE BASE - TOUT INCLUS):`);
      console.log(`   └─ Volume: ${volume}m³ × ${baseRate}€ = ${volumeCost.toFixed(2)}€`);
      console.log(`   └─ Cartons: ${numberOfBoxes.toFixed(1)} × ${boxPrice}€ = ${boxesCost.toFixed(2)}€`);
      console.log(`   └─ Fournitures premium: ${boxesCost.toFixed(2)}€ × ${suppliesMultiplier} = ${suppliesCost.toFixed(2)}€`);
      console.log(`   └─ Main d'œuvre premium: ${workers} déménageurs × ${duration}h × ${premiumLaborRate}€ = ${laborCost.toFixed(2)}€`);
      console.log(`   └─ Camion: ${truckRate}€ × ${numberOfDays} jour${numberOfDays > 1 ? 's' : ''} (${duration}h ÷ ${hoursPerDay}h/jour) = ${truckCost.toFixed(2)}€`);
      console.log(`   └─ Distance: ${chargeableKm}km × ${distanceRate}€ = ${distanceCost.toFixed(2)}€`);
      console.log(`   └─ Carburant: ${chargeableKm}km × ${fuelRate}€ = ${fuelCost.toFixed(2)}€`);
      console.log(`   └─ Péages: ${chargeableKm}km × ${tollRate}€ = ${tollCost.toFixed(2)}€`);
      console.log(`   └─ PRIX DE BASE MOVING_PREMIUM: ${baseTotal.toFixed(2)}€`);
    }
    ...
    console.log(`💰 [MOVING-STRATEGY] Prix de base après promotions: ${baseTotal.toFixed(2)}€`);
  ...
  private applyPromotionCodes(context: QuoteContext, basePrice: number): { finalPrice: number; details: { label: string; amount: number }[] } {
    ...
    console.log(`📊 [MOVING-STRATEGY] Type: ${promotionType}, Valeur: ${promotionValue}`);
    console.log(`💰 [MOVING-STRATEGY] Prix de base avant promotion: ${basePrice.toFixed(2)}€`);
    ...
    if (promotionType === 'PERCENT') {
      ...
      console.log(`✅ [MOVING-STRATEGY] Promotion pourcentage appliquée: -${promotionValue}% = -${discountAmount.toFixed(2)}€`);
      ...
      console.log(`✅ [MOVING-STRATEGY] Promotion fixe appliquée: -${promotionValue}€`);
    } else {
      console.log(`⚠️ [MOVING-STRATEGY] Type de promotion non reconnu: ${promotionType}`);
    }
    
    console.log(`💰 [MOVING-STRATEGY] Prix final après promotion: ${finalPrice.toFixed(2)}€`);
    return { finalPrice, details };
  }
  ...
  private calculatePriceWithDetails(context: QuoteContext, baseTotal: number): { total: number; details: { label: string; amount: number }[]; discounts: any[] } {
    ...
    console.log('🔧 ═══════════════════════════════════════════════════════════════');
    console.log(`💰 Prix de base reçu (après promotions): ${baseTotal.toFixed(2)}€`);
    console.log('🎯 ServiceType:', context.getServiceType());
    console.log('📋 Données contexte pour règles:', JSON.stringify(context.getAllData(), null, 2));

    // Vérifier les règles disponibles dans le RuleEngine
    const availableRules = this.ruleEngine.getRules();
    console.log(`📚 Nombre de règles disponibles dans RuleEngine: ${availableRules.length}`);

    if (availableRules.length > 0) {
      console.log('📋 Détail des règles disponibles:');
      availableRules.forEach((rule: any, index: number) => {
        console.log(`   ${index + 1}. "${rule.name}" - Type: ${rule.serviceType} - Valeur: ${rule.value} - PercentBased: ${rule.percentBased} - Active: ${rule.isActive}`);
      });
    } else {
    ...
    console.log(`💰 [MOVING-STRATEGY] Prix de base avant règles: ${baseTotal.toFixed(2)}€`);
    ...
    console.log(`💰 [MOVING-STRATEGY] Money object créé avec montant: ${baseMoneyAmount.getAmount()}€`);
    ...
    console.log(`   └─ Différence appliquée: ${(finalTotal - baseTotal).toFixed(2)}€`);
    ...
    if (ruleResult.discounts.length > 0) {
      ...
      ruleResult.discounts.forEach((discount: any, index: number) => {
        ...
        console.log(`      └─ Montant: ${ruleAmount > 0 ? '+' : ''}${ruleAmount.toFixed(2)}€`);
    ...
    console.log(`   └─ Pourcentage de changement: ${((finalTotal - baseTotal) / baseTotal * 100).toFixed(2)}%`);
  ...
  private isPackingUnchanged(context: QuoteContext): boolean {
    ...
    console.log('📋 Baseline récupérée (__presetSnapshot):', JSON.stringify(baseline, null, 2));
    console.log('🔍 Objectif: Déterminer si PACKING est inchangé pour utiliser defaultPrice');

    if (!baseline) {
      console.log('❌ [MOVING-STRATEGY] Pas de baseline trouvé, PACKING considéré comme modifié');
      console.log('🔍 [MOVING-STRATEGY] Raison: __presetSnapshot est undefined/null');
      return false;
    }
    ...
    for (const k of KEYS) {
      ...
      console.log(`  ${String(k)}: ${av} (${typeof av}) vs ${bv} (${typeof bv}) = ${equal ? '✅ Égal' : '❌ Différent'}`);

      if (!equal) {
        console.log(`❌ [MOVING-STRATEGY] Différence détectée sur ${String(k)}, PACKING modifié`);
    ...
    console.log('✅ [MOVING-STRATEGY] Aucune différence détectée, PACKING inchangé');
  ...
  private async getPricingConstants(): Promise<Record<string, number>> {
    try {
      ...
      const unifiedConfigs = await this.unifiedDataService.getAllPricingConstants(UnifiedServiceType.MOVING);
      if (Object.keys(unifiedConfigs).length > 0) {
        console.log(`✅ [MOVING-STRATEGY] ${Object.keys(unifiedConfigs).length} configurations récupérées depuis UnifiedDataService`);
        return unifiedConfigs;
      }
    } catch (error) {
      console.warn('⚠️ [MOVING-STRATEGY] Erreur UnifiedDataService, fallback vers ConfigurationService:', error);
    }

    try {
      // 2. Fallback vers ConfigurationService (si disponible)
      if (this.configService) {
        console.log('⚠️ [MOVING-STRATEGY] ConfigurationService non disponible, passage au fallback DefaultValues');
      }
    } catch (error) {
      console.warn('⚠️ [MOVING-STRATEGY] Erreur ConfigurationService, fallback vers DefaultValues:', error);
    }

    // 3. Dernier recours : DefaultValues
    console.log('⚠️ [MOVING-STRATEGY] Utilisation de DefaultValues comme fallback');
    return {
      ...
      'MOVING_BOXES_PER_M3': 10, // Valeur par défaut
      'MOVING_BOX_PRICE': 2, // Valeur par défaut
      'MOVING_WORKERS_PER_M3_THRESHOLD': 5, // Valeur par défaut
      'HOURS_PER_DAY': 7, // 7 heures de travail par jour par défaut
      'MOVING_PREMIUM_SUPPLIES_MULTIPLIER': 2.5, // Fournitures premium = cartons × 2.5
      'MOVING_PREMIUM_WORKER_PRICE_PER_HOUR': 40 // Taux horaire premium 40€
    };
  }
