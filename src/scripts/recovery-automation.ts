...
class RecoveryAutomationProcessor {
  ...
  async processRecoveryAutomation(): Promise<void> {
    ...
    try {
      ...
      this.logger.error(
        "❌ Erreur fatale lors du processus de récupération:",
        error,
      );
      throw error;
    }
  ...
  private async identifyAbandonedSessions(): Promise<AbandonedSession[]> {
    ...
    try {
      ...
      this.logger.error(
        "❌ Erreur lors de l'identification des abandons:",
        error,
      );
      throw error;
    }
  ...
  private async identifyQuoteAbandons(): Promise<AbandonedSession[]> {
    ...
    return quoteRequests.map((quote) => ({
      ...
      email:
        (quote.quoteData as any)?.email ||
        (quote.quoteData as any)?.customerInfo?.email,
      phone:
        (quote.quoteData as any)?.phone ||
        (quote.quoteData as any)?.customerInfo?.phone,
  ...
  private async applyRecoveryStrategies(
    ...
    for (const session of abandonedSessions) {
      try {
        ...
        totalErrors++;
        this.logger.error(
          `❌ Erreur lors de la récupération pour ${session.id}:`,
          error,
        );
      }
    }
  ...
  private async executeRecoveryStrategy(
    ...
    this.logger.info(
      `🎯 Exécution de la stratégie "${strategy.name}" pour ${session.id}`,
    );
    ...
    try {
      ...
      this.logger.info(
        `✅ Notification de récupération envoyée pour ${session.id}`,
      );
      return true;
    } catch (error) {
      this.logger.error(
        `❌ Erreur lors de l'envoi de notification pour ${session.id}:`,
        error,
      );
      return false;
    }
  ...
  private async sendRecoveryNotification(
    ...
    this.logger.info(
      `📧 Envoi de notification "${strategy.template}" à ${session.email || session.phone}`,
      {
