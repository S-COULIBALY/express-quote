...
export class NotificationTemplate {
  ...
  get variables(): ReadonlyMap<string, TemplateVariable> {
  ...
  updateVariable(name: string, updates: Partial<TemplateVariable>): void {
    ...
    if (!validationResult.success) {
      throw new Error(
        `Variable mise à jour invalide: ${validationResult.error.message}`,
      );
    }
   ...
   */
  private async compileEmailTemplate(
    language: SupportedLanguage,
    content: TemplateContent,
  ): Promise<void> {
    try {
      ...
      if (
        this._type === TemplateType.EMAIL_HTML &&
        this.isReactEmailTemplate()
      ) {
        ...
        if (hasReactEmailTemplate(normalizedId)) {
          ...
          const compiled = renderReactEmailTemplate(
            normalizedId,
            content.previewData || {},
          );
  ...
  private isReactEmailTemplate(): boolean {
    ...
    return (
      reactEmailTemplates.includes(this._id) ||
      reactEmailTemplates.includes(normalizedId)
    );
  }
  ...
  private async renderEmailTemplate(
    ...
    if (this.isReactEmailTemplate()) {
      ...
      console.log(
        "[NotificationTemplate] Rendering React Email template with real data",
      );
      console.log("[NotificationTemplate] Template ID (original):", this._id);
      console.log(
        "[NotificationTemplate] Template ID (normalized):",
        normalizedId,
      );
      console.log(
        "[NotificationTemplate] Variables keys:",
      ...
      try {
        ...
        if (hasReactEmailTemplate(normalizedId)) {
          ...
          console.log(
            "[NotificationTemplate] ========== APPEL RENDERER ==========",
          );
          console.log(
            "[NotificationTemplate] Calling renderReactEmailTemplate with:",
            normalizedId,
          );
          ...
          console.log(
            "[NotificationTemplate] ========== RÉSULTAT RENDERER ==========",
          );
          console.log("[NotificationTemplate] Compiled object:", {
            ...
            htmlPreview:
              typeof compiled?.html === "string"
                ? compiled.html.substring(0, 150)
                : "NOT A STRING",
          ...
          if (compiled && compiled.html && typeof compiled.html === "string") {
            console.log(
              "[NotificationTemplate] ✅ RETURNING React Email HTML (length:",
            ...
            console.error(
              "[NotificationTemplate] ❌ React Email renderer returned INVALID result!",
            );
            console.error(
              "[NotificationTemplate] compiled:",
              JSON.stringify(compiled, null, 2),
            );
          }
        } else {
          console.warn(
            `[NotificationTemplate] React Email template '${normalizedId}' not found in renderer`,
          );
        }
      } catch (error) {
        console.error(
          "[NotificationTemplate] Error rendering React Email:",
          error,
        );
  ...
  private validateContentByType(content: TemplateContent): void {
    switch (this._type) {
      ...
      case TemplateType.EMAIL_HTML:
        if (
          !/< *(?:html|div|p|h[1-6]|ul|ol|li|table|span|a|strong|em|br|img)\b/i.test(
            content.body,
          )
        ) {
  ...
  private recalculatePerformanceScore(): void {
    ...
    const speedScore =
      Math.max(0, 100 - this._metrics.avgCompilationTime / 100) * 30;
    const reliabilityScore =
      Math.max(0, 100 - this._metrics.renderErrorRate * 10) * 30;

    this._metrics.performanceScore = Math.round(
      deliveryScore + speedScore + reliabilityScore,
    );
  }
  ...
  toJSON(): any {
    return {
      ...
      variables: Array.from(this._variables.entries()).map(
        ([varName, variable]) => ({
          ...variable,
          name: varName,
        }),
          ...
          lang,
          {
            ...
            compilationMetadata: content.compilationMetadata
              ? {
                  ...
                  compiledAt:
                    content.compilationMetadata.compiledAt.toISOString(),
                }
              : undefined,
          },
        ]),
  ...
  static fromJSON(json: any): NotificationTemplate {
    ...
    for (const [lang, rawContent] of Object.entries(
      json.content as Record<string, Record<string, unknown>>,
    )) {
...
export class NotificationTemplateFactory {
  ...
  static createPaymentConfirmationTemplate(): NotificationTemplate {
    ...
    template.setContent(SupportedLanguage.FR, {
      version: "1.0.0",
      subject:
        "💳 Paiement confirmé {{payment.amount}}€ - Réservation {{booking.id}}",
      body: `
  ...
  static createProfessionalAttributionEmailTemplate(): NotificationTemplate {
    ...
    template.setContent(
      SupportedLanguage.FR,
      {
        version: "1.0.0",
        subject:
          "🎯 Nouvelle mission {{serviceType}} - {{totalAmount}}€ à {{locationCity}}",
        body: `
