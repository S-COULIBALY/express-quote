...
import {
  ...
  SupportedLanguage,
} from "../../core/entities/NotificationTemplate";
...
} from "../../infrastructure/events/modern.event.bus";
...
export class ProductionNotificationService {
  ...
  async initialize(): Promise<void> {
    ...
    try {
      ...
      this.eventBus.setupBusinessHandlers(
      ...
      this.logger.info(
        "✅ Production notification service initialized successfully",
      );
    } catch (error) {
  ...
  async sendNotification(
    ...
    try {
      ...
      await this.eventBus.emit("notification.created", {
        ...
        recipientId: notification.recipient,
        channel: notification.type.toUpperCase() as
          | "EMAIL"
          | "SMS"
          | "WHATSAPP",
        timestamp: new Date(),
        ...
        priority:
          (notification.priority?.toUpperCase() as
            | "LOW"
            | "NORMAL"
            | "HIGH"
            | "URGENT") || "NORMAL",
  ...
  async sendBulkNotifications(
    ...
    for (let i = 0; i < notifications.length; i += batchSize) {
      ...
      this.logger.info(
        `📤 Processing batch ${Math.floor(i / batchSize) + 1}/${Math.ceil(notifications.length / batchSize)}`,
      );
  ...
  private async processEmailNotification(
    ...
    const notificationId =
      (notification as any).notificationId || notification.id;
    ...
    try {
      ...
      if (notificationId) {
        try {
          ...
          this.logger.error("Failed to update notification status on error", {
            dbError,
          });
        }
      }
  ...
  private async processReminderNotification(
    ...
    try {
      this.logger.info(
        `📅 Processing reminder ${reminderData.reminderType} for booking ${reminderData.bookingId}`,
      );
      ...
      switch (reminderData.reminderType) {
        ...
        default:
          throw new Error(
            `Unknown reminder type: ${reminderData.reminderType}`,
          );
      }
      ...
      this.logger.info(
        `✅ Reminder ${reminderData.reminderType} processed for booking ${reminderData.bookingId}`,
        {
          success,
          messageIds,
          latencyMs,
      ...
      this.logger.error(
        `❌ Failed to process reminder ${reminderData.reminderType} for booking ${reminderData.bookingId}:`,
        {
          error: (error as Error).message,
          latencyMs,
        },
   ...
   */
  private async validateAndSanitizeNotification(
    notification: NotificationMessage,
  ): Promise<void> {
    // Validation de base
    if (!notification.id) {
    ...
    if (!allowedTypes.includes(notification.type)) {
      throw new Error(
        `Invalid notification type: ${notification.type}. Allowed: ${allowedTypes.join(", ")}`,
      );
    }
    ...
    if (
      ...
      !allowedPriorities.includes(notification.priority)
    ) {
      throw new Error(
        `Invalid priority: ${notification.priority}. Allowed: ${allowedPriorities.join(", ")}`,
      );
    }
    ...
    switch (notification.type) {
      ...
      case "sms":
        ...
        if (
          !phoneRegex.test(notification.recipient.replace(/[\s\-\(\)]/g, ""))
        ) {
        ...
        if (notification.content.length > 160) {
          this.logger.warn(
            `SMS content exceeds 160 characters (${notification.content.length}), may be split`,
            {
              notificationId: notification.id,
            },
          );
      ...
      case "whatsapp":
        ...
        if (
          !whatsappPhoneRegex.test(
            notification.recipient.replace(/[\s\-\(\)]/g, ""),
          )
        ) {
    ...
    try {
      switch (notification.type) {
        case "email":
          if (notification.subject) {
            notification.subject = await this.sanitizer.sanitizeSubject(
              notification.subject,
            );
          }
          notification.content = await this.sanitizer.sanitizeHtml(
            notification.content,
          );
          notification.recipient = await this.sanitizer.sanitizeEmail(
            notification.recipient,
          );
          break;

        case "sms":
          notification.content = await this.sanitizer.sanitizeText(
            notification.content,
          );
          notification.recipient = await this.sanitizer.sanitizePhoneNumber(
            notification.recipient,
          );
          break;
      ...
      throw new Error(
        `Content sanitization failed: ${(sanitizeError as Error).message}`,
      );
    }

    // Validation de sécurité finale
    try {
      const isValid = await this.sanitizer.validateForInjection(
        notification.content,
      );
      if (!isValid) {
      ...
      throw new Error(
        `Security validation failed: ${(securityError as Error).message}`,
      );
    }
  ...
  private async applyTemplate(
    ...
    const sophisticatedTemplate = this.templateRegistry.get(
      notification.templateId!,
    );

    this.logger.info(
      `📋 Template lookup result for '${notification.templateId}':`,
      {
    ...
    if (sophisticatedTemplate) {
      try {
        ...
        this.logger.info(
          `🎨 Rendering template '${notification.templateId}' with variables:`,
          {
            language,
        ...
        const renderResult = await sophisticatedTemplate.render(
          language,
          notification.variables || {},
        );

        this.logger.info(`✅ Template rendered successfully:`, {
          subjectLength: renderResult.subject.length,
          bodyLength: renderResult.body.length,
          hasTextBody: !!renderResult.textBody,
        });

        return {
          ...notification,
          content: renderResult.body,
          subject: renderResult.subject,
        };
      } catch (error) {
        this.logger.error(
          `❌ Failed to render sophisticated template '${notification.templateId}':`,
          {
            error: (error as Error).message,
      ...
      this.logger.warn(
        `❌ No sophisticated template found for '${notification.templateId}'`,
      );
    }
    ...
    if (legacyTemplate.type !== notification.type) {
      throw new Error(
        `Template type '${legacyTemplate.type}' doesn't match notification type '${notification.type}'`,
      );
    }
  ...
  private async sendReminder24h(
    ...
    try {
      ...
      this.logger.warn(
        `⚠️ Failed to send email reminder for booking ${reminderData.bookingId}:`,
        { error: (error as Error).message },
      );
    }
  ...
  private async sendReminder1h(
    ...
    if (!reminderData.customerPhone && reminderData.customerEmail) {
      ...
      this.logger.info(
        `📧 URGENT Email reminder 1h sent (fallback for no phone)`,
        {
          bookingId: reminderData.bookingId,
          email: reminderData.customerEmail,
        },
  ...
  private async getCustomerEmail(bookingId: string): Promise<string | null> {
    try {
      ...
      this.logger.info(
        `✅ Email found for booking ${bookingId}: ${customerEmail}`,
      );

      return customerEmail;
    } catch (error) {
      this.logger.error(
        `❌ Failed to get customer email for booking ${bookingId}:`,
        {
          error: (error as Error).message,
        },
      );
  ...
  private async getBookingData(bookingId: string): Promise<{
    ...
    try {
      ...
      const additionalInfo = booking.additionalInfo as Record<
        string,
        unknown
      > | null;
      const serviceDetails =
        (additionalInfo?.serviceDetails as Record<string, unknown>) || {};
      const addressInfo =
        (additionalInfo?.addressInfo as Record<string, unknown>) || {};
      ...
      const bookingData = {
        ...
        serviceName: String(
          serviceDetails.serviceName || booking.type || "Service Express Quote",
        ),
        serviceDate: String(
          serviceDetails.serviceDate || new Date().toISOString().split("T")[0],
        ),
  ...
  private async loadDefaultTemplatesFromFactory(): Promise<void> {
    try {
      ...
      const templates = [
        {
          id: "quote-confirmation-email",
          factory: () =>
            NotificationTemplateFactory.createQuoteConfirmationEmailTemplate(),
        },
        {
          id: "booking-confirmation-email",
          factory: () =>
            NotificationTemplateFactory.createBookingConfirmationEmailTemplate(),
        },
        {
          id: "payment-confirmation-email",
          factory: () =>
            NotificationTemplateFactory.createPaymentConfirmationEmailTemplate(),
        },
        {
          id: "appointment-reminder-sms",
          factory: () =>
            NotificationTemplateFactory.createAppointmentReminderTemplate(),
        },
        {
          id: "service-reminder-email",
          factory: () =>
            NotificationTemplateFactory.createServiceReminderEmailTemplate(),
        },
        {
          id: "professional-attribution-email",
          factory: () =>
            NotificationTemplateFactory.createProfessionalAttributionEmailTemplate(),
        },
      ];
      ...
      for (const { id, factory } of templates) {
        ...
        this.logger.debug(
          `📄 Template '${id}' created from factory (cache disabled)`,
        );
        ...
        this.logger.info(`🔧 Added template '${id}' to registry:`, {
          ...
          templateClass:
            this.templateRegistry.get(id)?.constructor?.name || "undefined",
          totalSize: this.templateRegistry.size,
        });
      }

      // Alias pour compatibilité avec les anciennes références
      const quoteConfirmationEmail = this.templateRegistry.get(
        "quote-confirmation-email",
      );
      const bookingConfirmationEmail = this.templateRegistry.get(
        "booking-confirmation-email",
      );
      const paymentConfirmationEmail = this.templateRegistry.get(
        "payment-confirmation-email",
      );
      const appointmentReminderSms = this.templateRegistry.get(
        "appointment-reminder-sms",
      );
      const serviceReminderEmail = this.templateRegistry.get(
      ...
      if (quoteConfirmationEmail) {
        ...
        this.logger.info(
          "✅ Alias quote-confirmation créé pour quote-confirmation-email",
        );
      } else {
        this.logger.error(
          "❌ Template quote-confirmation-email non trouvé pour créer l'alias",
        );
      }

      if (bookingConfirmationEmail) {
        this.templateRegistry.set(
          "booking-confirmation",
          bookingConfirmationEmail,
        );
        this.logger.info(
          "✅ Alias booking-confirmation créé pour booking-confirmation-email",
        );
      } else {
        this.logger.error(
          "❌ Template booking-confirmation-email non trouvé pour créer l'alias",
        );
      }
      ...
      if (paymentConfirmationEmail) {
        ...
        this.logger.info(
          "✅ Alias payment-confirmation créé pour payment-confirmation-email",
        );
      } else {
        this.logger.error(
          "❌ Template payment-confirmation-email non trouvé pour créer l'alias",
        );
      }
      ...
      if (serviceReminderEmail) {
        ...
        this.logger.info(
          "✅ Alias service-reminder créé pour service-reminder-email",
        );
      } else {
        this.logger.error(
          "❌ Template service-reminder-email non trouvé pour créer l'alias",
        );
      }

      const professionalAttributionEmail = this.templateRegistry.get(
        "professional-attribution-email",
      );
      if (professionalAttributionEmail) {
        this.templateRegistry.set(
          "professional-attribution",
          professionalAttributionEmail,
        );
        this.logger.info(
          "✅ Alias professional-attribution créé pour professional-attribution-email",
        );
      } else {
        this.logger.error(
          "❌ Template professional-attribution-email non trouvé pour créer l'alias",
        );
      }
      ...
      this.logger.info("📄 Final template registry state:", {
        ...
        bookingConfirmationExists: this.templateRegistry.has(
          "booking-confirmation",
        ),
        bookingConfirmationEmailExists: this.templateRegistry.has(
          "booking-confirmation-email",
        ),
      });

      this.logger.info(
        `✅ Loaded ${this.templateRegistry.size} sophisticated templates (with caching)`,
      );
    } catch (error) {
      this.logger.error("❌ Failed to load sophisticated templates:", {
        error: (error as Error).message,
      });
    }
  }
  ...
  private loadLegacyTemplates(): void {
    ...
    this.legacyTemplates.set("confirmation-sms", {
      ...
      type: "sms",
      content:
        "Votre réservation {{bookingId}} est confirmée pour le {{date}}. Merci !",
      variables: ["bookingId", "date"],
      active: true,
    });

    this.logger.info(
      `✅ Loaded ${this.legacyTemplates.size} legacy templates for compatibility`,
    );
  }
  ...
  async getServiceStats(): Promise<any> {
    ...
    return {
      ...
      templates: {
        ...
        activeLegacy: Array.from(this.legacyTemplates.values()).filter(
          (t) => t.active,
        ).length,
      },
  ...
  async sendEmailWithFallback(options: {
    ...
    try {
      ...
      this.logger.warn(
        `⚠️ Failed to use primary template '${options.primaryTemplate}', falling back to '${options.fallbackTemplate}'`,
        {
          error: (error as Error).message,
      ...
      try {
        ...
        return await this.sendSimpleEmail({
          ...
          subject:
            options.subject ||
            `Rappel de service - ${options.data?.serviceName || "Express Quote"}`,
  ...
  async sendWhatsApp(options: {
    ...
    const message: NotificationMessage = {
      ...
      variables: options.template?.params
...
- Service : ${serviceName}
- Date : ${serviceDate}${serviceTime ? ` à ${serviceTime}` : ""}${
      serviceAddress
        ? `
...
<body>
  ...
  async sendBookingConfirmation(
    ...
    if (data.customerPhone || email) {
      try {
        ...
        this.logger.info(
          `📅 Reminders scheduled for booking ${data.bookingId}`,
          {
        ...
        this.logger.error(
          `❌ Failed to schedule reminders for booking ${data.bookingId}:`,
          { error: (error as Error).message },
        );
      ...
      this.logger.info(
        `⏭️ No phone number provided for booking ${data.bookingId}, skipping reminders`,
      );
    }
  ...
  async sendServiceReminder(
    ...
    try {
      this.logger.info("📅 Sending service reminder", {
        email,
        bookingId: data.bookingId,
      });
      ...
      const serviceDateTime = new Date(
        `${data.reminderDetails.appointmentDate}T${data.reminderDetails.appointmentTime}`,
      );
      const now = new Date();
      ...
      this.logger.info("✅ Service reminder sent successfully", {
        id: result.id,
        email,
      });
      return result;
    } catch (error) {
      this.logger.error("❌ Failed to send service reminder", {
        error: (error as Error).message,
      });
      return {
  ...
  async sendPasswordResetNotification(
    email: string,
    data: { resetToken: string; [key: string]: any },
  ): Promise<NotificationResult> {
    return await this.sendEmail({
      to: email,
   ...
   */
  async sendMovingConfirmationSMS(
    phoneNumber: string,
    data: {
      customerName: string;
      bookingId: string;
   ...
   */
  async sendCleaningConfirmationSMS(
    phoneNumber: string,
    data: {
      customerName: string;
      bookingId: string;
  ...
  async scheduleBookingReminders(bookingData: {
    ...
    try {
      ...
      if (!bookingData.bookingId || !bookingData.customerName) {
        throw new Error(
          "Missing required booking data: bookingId or customerName",
        );
      }

      if (!bookingData.customerPhone && !bookingData.customerEmail) {
        throw new Error(
          "Either customerPhone or customerEmail must be provided for reminders",
        );
      }

      if (!bookingData.serviceDate || !bookingData.serviceTime) {
        throw new Error(
          "Missing required service scheduling data: serviceDate or serviceTime",
        );
      }

      // Validation du format de date
      const serviceDateTime = new Date(
        `${bookingData.serviceDate}T${bookingData.serviceTime}`,
      );
      if (isNaN(serviceDateTime.getTime())) {
        throw new Error(
          `Invalid service date/time format: ${bookingData.serviceDate}T${bookingData.serviceTime}`,
        );
      }
      ...
      if (serviceDateTime <= new Date()) {
        throw new Error(
          `Service date/time is in the past: ${serviceDateTime.toISOString()}`,
        );
      }

      this.logger.info(
        `📅 Scheduling reminders for booking ${bookingData.bookingId}`,
        {
      ...
      for (const reminder of reminders) {
        try {
          const scheduledDate = new Date(
            serviceDateTime.getTime() - reminder.hours * 60 * 60 * 1000,
          );
          ...
          if (scheduledDate > new Date(now.getTime() + safetyMargin)) {
            ...
            scheduledReminders.push(
              `${reminder.type} (${scheduledDate.toISOString()})`,
            );
            this.logger.info(
              `✅ Reminder ${reminder.type} scheduled for booking ${bookingData.bookingId}`,
              {
            ...
            this.logger.warn(
              `⏭️ Skipping reminder ${reminder.type} for booking ${bookingData.bookingId} (${skipReason})`,
              {
                scheduledFor: scheduledDate.toISOString(),
      ...
      // Log final result
      this.logger.info(
        `📊 Reminder scheduling completed for booking ${bookingData.bookingId}`,
        {
      ...
      this.logger.error(
        `❌ Failed to schedule reminders for booking ${bookingData.bookingId}:`,
        {
          error: (error as Error).message,
          stack: (error as Error).stack,
        },
  ...
  private async scheduleReminder(reminderData: ReminderJobData): Promise<void> {
    ...
    await this.queueManager.addJob(
      "reminders",
      `reminder-${reminderData.reminderType}`,
      {
      ...
      },
      {
        delay,
        priority:
          reminderData.reminderType === "1h"
            ? 1
            : reminderData.reminderType === "24h"
              ? 5
              : 10,
    ...
    try {
      ...
      for (const job of jobs) {
        if (job.data.bookingId === bookingId) {
          ...
          cancelledCount++;
          this.logger.info(
            `🗑️ Cancelled reminder ${job.data.reminderType} for booking ${bookingId}`,
          );
        }
      }

      this.logger.info(
        `✅ Cancelled ${cancelledCount} reminders for booking ${bookingId}`,
      );
   ...
   */
  async rescheduleBookingReminders(
    bookingId: string,
    newServiceDate: string,
    newServiceTime: string,
  ): Promise<{ success: boolean; rescheduledCount: number }> {
    try {
      ...
      this.logger.error(
        `❌ Failed to reschedule reminders for booking ${bookingId}:`,
        { error: (error as Error).message },
      );
      return {
        success: false,
   ...
   */
  async getNotificationByExternalId(
    externalId: string,
    ...
    try {
      ...
      if (!result.success || !result.result) {
        this.logger.info("🔍 Notification non trouvée par ID externe", {
          externalId,
        });
        return null;
      }
      ...
      const notificationMessage: NotificationMessage = {
        ...
        type: dbNotification.channel.toLowerCase() as
          | "email"
          | "sms"
          | "whatsapp",
        ...
        variables:
          (dbNotification.templateData as Record<string, any>) || undefined,
        priority: dbNotification.priority.toLowerCase() as
          | "low"
          | "normal"
          | "high"
          | "critical",
      ...
      this.logger.error("❌ Erreur recherche par ID externe", {
        externalId,
        error,
      });
      throw error;
  ...
  async cancelNotification(
    ...
    try {
      this.logger.info(`🗑️ Cancelling notification ${notificationId}`, {
        reason,
      });

      // 1. Marquer la notification comme annulée en base de données
      try {
        await this.repository.markAsCancelled(
          notificationId,
          reason || "Manual cancellation",
        );
      } catch (dbError) {
        this.logger.warn("Failed to update notification status in DB", {
      ...
      for (const queueName of queueNames) {
        try {
          ...
          for (const job of waitingJobs) {
            if (
              job.data.id === notificationId ||
              job.data.notificationId === notificationId
            ) {
              await job.remove();
              jobFound = true;
              this.logger.info(
                `✅ Job removed from queue '${queueName}' for notification ${notificationId}`,
              );
              break;
            }
          }
          ...
          this.logger.warn(
            `Failed to search/remove job from queue '${queueName}'`,
            {
      ...
      if (!jobFound) {
        this.logger.warn(
          `⚠️ No active job found for notification ${notificationId} (may already be processed)`,
        );
      }
  ...
  async retryNotification(notificationId: string): Promise<NotificationResult> {
    ...
    try {
      this.logger.info("🔄 Tentative de relance de notification", {
        notificationId,
      });
  ...
  async healthCheck(): Promise<{
    ...
    try {
      ...
      this.logger.error("❌ Erreur lors du health check global", {
        error: (error as Error).message,
      });
    }
  ...
  async cleanupExpiredNotifications() {
    try {
      ...
      if (expiredNotifications.length > 0) {
        this.logger.info(
          `🗑️ ${expiredNotifications.length} notifications expirées nettoyées`,
        );
      }
    ...
    try {
      ...
      for (const job of failedJobs) {
        try {
          ...
          this.logger.warn(`Failed to remove job ${job.id}`, {
            error: (error as Error).message,
          });
        }
      }
      ...
      this.logger.info(
        `✅ Cleaned ${cleanedCount} failed jobs from queue '${queueName}'`,
      );
    } catch (error) {
      this.logger.error(
        `❌ Error cleaning failed jobs from queue '${queueName}'`,
        { error: (error as Error).message },
      );
      throw error;
    }
  ...
  async shutdown(): Promise<void> {
    ...
    try {
      await Promise.all([this.queueManager.close(), this.eventBus.shutdown()]);
