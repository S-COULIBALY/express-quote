...
export function loadEnv() {
  ...
  console.log("Chargement des variables d'environnement:", {
    hasStripePublicKey: Boolean(stripePublicKey),
    stripePublicKeyLength: stripePublicKey.length,
  ...
  if (!env.NEXT_PUBLIC_STRIPE_PUBLISHABLE_KEY) {
    console.warn(
      "⚠️ NEXT_PUBLIC_STRIPE_PUBLISHABLE_KEY manquante. Le paiement ne fonctionnera pas correctement.",
    );
  }

  if (!env.STRIPE_SECRET_KEY) {
    console.warn(
      "⚠️ STRIPE_SECRET_KEY manquante. Le paiement ne fonctionnera pas correctement.",
    );
  }
