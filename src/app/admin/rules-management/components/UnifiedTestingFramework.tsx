...
export function UnifiedTestingFramework() {
  ...
  const predefinedScenarios: TestScenario[] = [
    {
      ...
      description: 'Pack déménagement T2 local 15km, 2 ouvriers, 7h, avec ascenseur à l\'arrivée',
      serviceType: 'PACKING',
      input: {
        serviceType: 'PACKING',
    ...
    },
    {
      ...
      description: 'Pack déménagement T4 avec escaliers difficiles, couloirs étroits, parking difficile, meubles encombrants et emballage pro',
      serviceType: 'PACKING',
      input: {
        ...
        logisticsConstraints: {
          ...
          deliveryLogisticsConstraints: ['difficult_parking', 'long_carrying_distance'],
    ...
    },
    {
      ...
      description: 'Déménagement sur mesure villa 85m³ avec piano, objets d\'art, meubles fragiles, emballage complet et stockage temporaire - PROMO 10%',
      serviceType: 'MOVING',
      input: {
        serviceType: 'MOVING',
        volume: 85,
        ...
        logisticsConstraints: {
          ...
          deliveryLogisticsConstraints: ['pedestrian_zone', 'administrative_permit'],
          services: [
            'transport_piano',
            'artwork_packing',
    ...
    },
    {
      ...
      description: 'Pack déménagement studio étudiant, 2 ouvriers, 4h, courte distance, pas de contraintes - PROMO 50€',
      serviceType: 'PACKING',
      input: {
        serviceType: 'PACKING',
    ...
    },
    {
      ...
      description: 'Déménagement sur mesure T3 45m³ sur longue distance (120km) avec emballage et services complémentaires',
      serviceType: 'MOVING',
      input: {
        serviceType: 'MOVING',
        volume: 45,
        ...
        logisticsConstraints: {
          ...
          deliveryLogisticsConstraints: ['difficult_stairs', 'narrow_inaccessible_street'],
          services: [
            'professional_packing_departure',
            'packing_supplies',
            'heavy_items',
    ...
    },
    {
      ...
      description: 'Déménagement sur mesure T3 40m³ au 5ème étage sans ascenseur, nécessite monte-meuble - PROMO 15%',
      serviceType: 'MOVING',
      input: {
        serviceType: 'MOVING',
        volume: 40,
        ...
        promotionValue: 15,
        logisticsConstraints: {
          pickupLogisticsConstraints: ['furniture_lift_required', 'difficult_stairs', 'narrow_corridors'],
          deliveryLogisticsConstraints: ['difficult_stairs', 'narrow_corridors'],
    ...
    },
    {
      ...
      description: 'Nettoyage approfondi maison 120m², 2 personnes, 6h avec services premium',
      serviceType: 'CLEANING',
      input: {
        serviceType: 'CLEANING',
        defaultPrice: 280,
  ...
  const runSingleTest = async (scenario: TestScenario) => {
    ...
    try {
      ...
      const result = await calculatePrice(scenario.input)
      ...
      const variancePercentage = expectedPrice > 0 ? (variance / expectedPrice) * 100 : 0

      let recommendation: 'accept' | 'review' | 'reject' = 'accept'
      if (variancePercentage > scenario.tolerance) {
        recommendation = variancePercentage > scenario.tolerance * 2 ? 'reject' : 'review'
      }
      ...
      toast({
        ...
        variant: testResult.success && recommendation === 'accept' ? "default" : "destructive"
      })
  ...
  const getTestSummary = () => {
    ...
    const passed = scenarios.filter(s => s.lastResult?.success && s.lastResult?.recommendation === 'accept').length
    const failed = scenarios.filter(s => s.lastResult?.success === false || s.lastResult?.recommendation === 'reject').length
  ...
  const getConstraintName = (constraintId: string, serviceType: string) => {
    const constraints = serviceType === 'MOVING' ? movingConstraintsFallback : cleaningConstraintsFallback
    ...
    return constraintId.replace(/_/g, ' ').replace(/\b\w/g, l => l.toUpperCase())
  }

  const getServiceName = (serviceId: string, serviceType: string) => {
    const services = serviceType === 'MOVING' ? movingServicesFallback : cleaningServicesFallback
    ...
    <div className="space-y-6 p-6 bg-gradient-to-br from-slate-50 to-purple-50 min-h-screen">
        ...
        <div className="flex justify-between items-start">
          <div>
            <div className="flex items-center gap-3 mb-2">
              ...
              <h3 className="text-2xl font-bold text-slate-800">Framework de Tests Unifié</h3>
            </div>
            <p className="text-sm text-slate-600">
              Testez vos règles de pricing en temps réel avec des scénarios personnalisés
            </p>
          </div>
          ...
          <div className="grid grid-cols-4 gap-4">
            ...
            <div className="text-center bg-orange-50 rounded-lg p-3 border border-orange-200">
              <div className="text-2xl font-bold text-orange-600">{scenarios.length - summary.tested}</div>
              <div className="text-xs text-orange-700 font-medium">À tester</div>
            </div>
          </div>
        </div>
      </div>
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
        <TabsList>
          ...
          <TabsTrigger value="scenarios">Scénarios ({scenarios.length})</TabsTrigger>
        ...
        <TabsContent value="create" className="space-y-4">
          ...
          <Card className="border-purple-200 shadow-sm bg-white">
            <CardHeader className="bg-gradient-to-r from-purple-50 to-pink-50 border-b border-purple-200">
              ...
              <CardDescription className="text-slate-600">Créez un scénario pour tester vos règles de pricing</CardDescription>
            </CardHeader>
            <CardContent className="space-y-6 pt-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-1.5">
                  <Label htmlFor="name" className="text-sm font-medium text-slate-700">Nom du scénario</Label>
                  <Input
                    id="name"
                ...
                <div className="space-y-1.5">
                  <Label htmlFor="serviceType" className="text-sm font-medium text-slate-700">Type de service</Label>
                  <Select
                  ...
                  >
                    ...
                    <SelectContent>
                      <SelectItem value="MOVING">Déménagement Sur Mesure</SelectItem>
              ...
              <div className="bg-slate-50 rounded-lg p-4 border border-slate-200">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  ...
                  {['CLEANING', 'PACKING', 'DELIVERY'].includes(newScenario.serviceType || '') && (
                    <div className="space-y-1.5">
                      <Label htmlFor="defaultPrice" className="text-sm font-medium text-slate-700 flex items-center gap-1">
                      ...
                      <Input
                        ...
                        onChange={(e) => setNewScenario(prev => ({
                          ...prev,
                          input: { ...prev.input!, defaultPrice: parseFloat(e.target.value) || 0 }
                        }))}
                        className="bg-white border-slate-300 focus:border-purple-500 focus:ring-purple-500"
                        placeholder="Ex: 120"
                        required
                  ...
                  {newScenario.serviceType === 'MOVING' && (
                    <div className="space-y-1.5">
                      <Label htmlFor="volume" className="text-sm font-medium text-slate-700 flex items-center gap-1">
                      ...
                      <Input
                        ...
                        onChange={(e) => setNewScenario(prev => ({
                          ...prev,
                          input: { ...prev.input!, volume: parseFloat(e.target.value) || 0 }
                        }))}
                        className="bg-white border-slate-300 focus:border-purple-500 focus:ring-purple-500"
                        placeholder="Ex: 30"
                        required
                  ...
                  <div className="space-y-1.5">
                    ...
                    <Input
                      ...
                      onChange={(e) => setNewScenario(prev => ({
                        ...prev,
                        input: { ...prev.input!, distance: parseFloat(e.target.value) || 0 }
                      }))}
                      className="bg-white border-slate-300 focus:border-purple-500 focus:ring-purple-500"
                      placeholder="Ex: 25"
                    />
                  ...
                  {['PACKING', 'CLEANING', 'DELIVERY'].includes(newScenario.serviceType || '') && (
                    <>
                      <div className="space-y-1.5">
                        <Label htmlFor="workers" className="text-sm font-medium text-slate-700 flex items-center gap-1">
                        ...
                        <Input
                          ...
                          onChange={(e) => setNewScenario(prev => ({
                            ...prev,
                            input: { ...prev.input!, workers: parseInt(e.target.value) || 1 }
                          }))}
                          className="bg-white border-slate-300 focus:border-purple-500 focus:ring-purple-500"
                          placeholder="Ex: 2"
                          required
                      ...
                      <div className="space-y-1.5">
                        <Label htmlFor="duration" className="text-sm font-medium text-slate-700 flex items-center gap-1">
                          Durée (heures) <span className="text-red-500">*</span> <span className="text-xs text-slate-500">(7h par défaut)</span>
                        </Label>
                        <Input
                          id="duration"
                          type="number"
                          ...
                          onChange={(e) => setNewScenario(prev => ({
                            ...prev,
                            input: { ...prev.input!, duration: parseFloat(e.target.value) || 7 }
                          }))}
                          className="bg-white border-slate-300 focus:border-purple-500 focus:ring-purple-500"
                          placeholder="Ex: 7"
                          required
                  ...
                  <div className="space-y-1.5">
                    <Label htmlFor="expectedPrice" className="text-sm font-medium text-slate-700">Prix attendu (€)</Label>
                    <Input
                      id="expectedPrice"
                      type="number"
                      ...
                      onChange={(e) => setNewScenario(prev => ({
                        ...prev,
                        expectedPrice: parseFloat(e.target.value) || undefined
                      }))}
                      className="bg-white border-slate-300 focus:border-purple-500 focus:ring-purple-500"
                      placeholder="Ex: 1200"
                    />
                  ...
                  <div className="mt-4 p-4 bg-gradient-to-br from-blue-50 to-indigo-50 rounded-lg border border-blue-200">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      ...
                      <div className="space-y-3">
                        <h6 className="text-sm font-semibold text-blue-700 border-b border-blue-200 pb-1">📍 Point de Départ</h6>
                        <div className="space-y-3">
                          <div className="space-y-1.5">
                            <Label htmlFor="pickupFloor" className="text-sm font-medium text-slate-700">Étage départ</Label>
                            <Input
                              id="pickupFloor"
                              type="number"
                              ...
                              onChange={(e) => setNewScenario(prev => ({
                                ...prev,
                                input: { ...prev.input!, pickupFloor: parseInt(e.target.value) || 0 }
                              }))}
                          ...
                          <div className="space-y-1.5">
                            <Label htmlFor="pickupElevator" className="text-sm font-medium text-slate-700">Ascenseur départ</Label>
                            <Select
                              value={newScenario.input?.pickupElevator?.toString() || 'false'}
                              onValueChange={(value) => setNewScenario(prev => ({
                                ...prev,
                                input: { ...prev.input!, pickupElevator: value === 'true' }
                              }))}
                            >
                          ...
                          <div className="space-y-1.5">
                            <Label htmlFor="pickupCarryDistance" className="text-sm font-medium text-slate-700">Distance de portage (m)</Label>
                            <Input
                              id="pickupCarryDistance"
                              type="number"
                              value={newScenario.input?.pickupCarryDistance ?? ''}
                              onChange={(e) => setNewScenario(prev => ({
                                ...prev,
                                input: { ...prev.input!, pickupCarryDistance: parseInt(e.target.value) || 0 }
                              }))}
                              className="bg-white border-slate-300 focus:border-blue-500 focus:ring-blue-500"
                              placeholder="Ex: 20"
                            />
                      ...
                      {/* Arrivée */}
                      <div className="space-y-3">
                        <h6 className="text-sm font-semibold text-blue-700 border-b border-blue-200 pb-1">🎯 Point d'Arrivée</h6>
                        <div className="space-y-3">
                          <div className="space-y-1.5">
                            <Label htmlFor="deliveryFloor" className="text-sm font-medium text-slate-700">Étage arrivée</Label>
                            <Input
                              id="deliveryFloor"
                              type="number"
                              ...
                              onChange={(e) => setNewScenario(prev => ({
                                ...prev,
                                input: { ...prev.input!, deliveryFloor: parseInt(e.target.value) || 0 }
                              }))}
                              className="bg-white border-slate-300 focus:border-blue-500 focus:ring-blue-500"
                              placeholder="Ex: 2"
                            />
                          ...
                          <div className="space-y-1.5">
                            <Label htmlFor="deliveryElevator" className="text-sm font-medium text-slate-700">Ascenseur arrivée</Label>
                            <Select
                              value={newScenario.input?.deliveryElevator?.toString() || 'false'}
                              onValueChange={(value) => setNewScenario(prev => ({
                                ...prev,
                                input: { ...prev.input!, deliveryElevator: value === 'true' }
                              }))}
                            >
                              <SelectTrigger className="bg-white border-slate-300">
                                <SelectValue />
                          ...
                          <div className="space-y-1.5">
                            <Label htmlFor="deliveryCarryDistance" className="text-sm font-medium text-slate-700">Distance de portage (m)</Label>
                            <Input
                              id="deliveryCarryDistance"
                              type="number"
                              value={newScenario.input?.deliveryCarryDistance ?? ''}
                              onChange={(e) => setNewScenario(prev => ({
                                ...prev,
                                input: { ...prev.input!, deliveryCarryDistance: parseInt(e.target.value) || 0 }
                              }))}
                              className="bg-white border-slate-300 focus:border-blue-500 focus:ring-blue-500"
                              placeholder="Ex: 15"
                            />
                ...
                <div className="mt-4 p-4 bg-gradient-to-br from-green-50 to-emerald-50 rounded-lg border border-green-200">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                    <div className="space-y-1.5">
                      <Label htmlFor="isPromotionActive" className="text-sm font-medium text-slate-700">Active</Label>
                      <Select
                        value={newScenario.input?.isPromotionActive ? 'true' : 'false'}
                        onValueChange={(value) => setNewScenario(prev => ({
                          ...prev,
                          input: { ...prev.input!, isPromotionActive: value === 'true' }
                        }))}
                      >
                        <SelectTrigger className="bg-white border-slate-300">
                          <SelectValue />
                    ...
                    <div className="space-y-1.5">
                      <Label htmlFor="promotionCode" className="text-sm font-medium text-slate-700">Code promo</Label>
                      <Input
                        id="promotionCode"
                        type="text"
                        ...
                        onChange={(e) => setNewScenario(prev => ({
                          ...prev,
                          input: { ...prev.input!, promotionCode: e.target.value }
                        }))}
                    ...
                    <div className="space-y-1.5">
                      <Label htmlFor="promotionType" className="text-sm font-medium text-slate-700">Type</Label>
                      <Select
                    ...
                    <div className="space-y-1.5">
                      <Label htmlFor="promotionValue" className="text-sm font-medium text-slate-700">
                        Valeur {newScenario.input?.promotionType === 'PERCENT' ? '(%)' : '(€)'}
                      </Label>
                      <Input
                        id="promotionValue"
                        type="number"
                        ...
                        onChange={(e) => setNewScenario(prev => ({
                          ...prev,
                          input: { ...prev.input!, promotionValue: parseFloat(e.target.value) || 0 }
                        }))}
                        className="bg-white border-slate-300 focus:border-green-500 focus:ring-green-500"
                        placeholder={newScenario.input?.promotionType === 'PERCENT' ? 'Ex: 10' : 'Ex: 50'}
                        disabled={!newScenario.input?.isPromotionActive}
                      />
                    </div>
                  </div>
              ...
              <div className="bg-gradient-to-br from-orange-50 to-red-50 rounded-lg p-4 border border-orange-200">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2 max-h-48 overflow-y-auto bg-white rounded-lg p-3 border border-orange-200">
                  {(newScenario.serviceType === 'MOVING' ? movingConstraintsFallback : cleaningConstraintsFallback).map(constraint => (
                    <label
                      key={constraint.id}
                    ...
                    >
                      <input
                        type="checkbox"
                        checked={newScenario.input?.constraints?.includes(constraint.id) || false}
                        onChange={(e) => {
                          const constraints = newScenario.input?.constraints || []
                          const newConstraints = e.target.checked
                            ? [...constraints, constraint.id]
                            : constraints.filter(id => id !== constraint.id)
                          setNewScenario(prev => ({
                            ...prev,
                            input: { ...prev.input!, constraints: newConstraints }
                          }))
                        }}
                      ...
                      <span className="text-sm text-slate-700 flex items-center gap-1">
                        ...
                        {constraint.value && (
                          <span className="text-xs text-orange-600 font-medium">+{constraint.value}€</span>
                        )}
                      </span>
                    </label>
                  ))}
              ...
              <div className="bg-gradient-to-br from-blue-50 to-indigo-50 rounded-lg p-4 border border-blue-200">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2 max-h-48 overflow-y-auto bg-white rounded-lg p-3 border border-blue-200">
                  {(newScenario.serviceType === 'MOVING' ? movingServicesFallback : cleaningServicesFallback).map(service => (
                    <label
                      key={service.id}
                    ...
                    >
                      <input
                        type="checkbox"
                        checked={newScenario.input?.services?.includes(service.id) || false}
                        onChange={(e) => {
                      ...
                      <span className="text-sm text-slate-700 flex items-center gap-1">
                        ...
                        {service.value && (
                          <span className="text-xs text-blue-600 font-medium">+{service.value}€</span>
                        )}
                      </span>
                    </label>
                  ))}
              ...
              <Button onClick={createNewScenario} className="w-full bg-purple-600 hover:bg-purple-700 text-white shadow-sm transition-all duration-200 hover:shadow-md">
        ...
        <TabsContent value="scenarios" className="space-y-4">
          ...
          <div className="grid gap-4">
            {scenarios.map(scenario => {
              ...
              return (
                <Card key={scenario.id} className="border-slate-200 shadow-sm bg-white transition-all duration-200 hover:shadow-md">
                    ...
                    <div className="space-y-4">
                            ...
                            {result && (
                              <Badge
                                className={`font-semibold ${
                                  result.success && result.recommendation === 'accept'
                              ...
                              >
                                {result.recommendation === 'accept' ? '✅ Validé' :
                                 result.recommendation === 'review' ? '⚠️ À réviser' : '❌ Rejeté'}
                              </Badge>
                            )}
                          </div>
                          ...
                          <div className="flex flex-wrap gap-2 mb-2">
                            {scenario.input.constraints?.length > 0 && (
                              <Badge variant="outline" className="bg-orange-50 border-orange-300 text-orange-700 text-xs">
                                🚧 {scenario.input.constraints.length} contraintes
                              </Badge>
                            )}
                      ...
                      {result && (
                        <div className="flex items-center gap-6 text-sm bg-slate-50 rounded-lg p-3 border border-slate-200">
                          <div className="flex items-center gap-1.5">
                            ...
                            <strong className="text-blue-700">{result.calculatedPrice}€</strong>
                          </div>
                          {result.expectedPrice && (
                            <div className="flex items-center gap-1.5">
                              <span className="text-slate-600">Attendu:</span>
                              <strong className="text-purple-700">{result.expectedPrice}€</strong>
                            </div>
                          )}
                          <div className="flex items-center gap-1.5">
                            ...
                            <strong className={`${
                              !scenario.expectedPrice
                                ? 'text-slate-500'
                                : result.variancePercentage <= scenario.tolerance
                                ? 'text-green-600'
                                : 'text-red-600'
                            }`}>
                              {!scenario.expectedPrice || isNaN(result.variancePercentage)
                                ? 'N/A'
                      ...
                      {isExpanded && (
                        <div className="space-y-4 pt-4 border-t border-slate-200">
                            ...
                            <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-sm">
                              {scenario.input.volume !== undefined && (
                                <div>
                                  ...
                                  <strong className="ml-2 text-slate-800">{scenario.input.volume} m³</strong>
                                </div>
                              )}
                              {scenario.input.distance !== undefined && (
                                <div>
                                  <span className="text-slate-600">Distance:</span>
                                  <strong className="ml-2 text-slate-800">{scenario.input.distance} km</strong>
                                </div>
                              )}
                              {scenario.input.workers !== undefined && (
                                <div>
                                  <span className="text-slate-600">Ouvriers:</span>
                                  <strong className="ml-2 text-slate-800">{scenario.input.workers}</strong>
                                </div>
                              )}
                              {scenario.input.duration !== undefined && (
                                <div>
                                  <span className="text-slate-600">Durée:</span>
                                  <strong className="ml-2 text-slate-800">{scenario.input.duration}h</strong>
                                </div>
                              )}
                            </div>
                          </div>
                          ...
                          {scenario.input.constraints && scenario.input.constraints.length > 0 && (
                            <div className="bg-gradient-to-br from-orange-50 to-red-50 rounded-lg p-4 border border-orange-200">
                              <h5 className="font-semibold text-orange-800 mb-3 flex items-center gap-2">
                                <div className="h-1 w-1 bg-orange-500 rounded-full"></div>
                                Contraintes Sélectionnées ({scenario.input.constraints.length})
                              </h5>
                              <div className="flex flex-wrap gap-2">
                                {scenario.input.constraints.map(constraintId => {
                                  const constraint = getConstraintData(constraintId, scenario.serviceType)
                                  const name = getConstraintName(constraintId, scenario.serviceType)
                                  return (
                                    <Badge key={constraintId} variant="outline" className="bg-white border-orange-300 text-orange-800 text-xs">
                                      {constraint?.icon && <span className="mr-1">{constraint.icon}</span>}
                                      {name}
                                      {constraint?.value && <span className="ml-1 font-bold">+{constraint.value}€</span>}
                                    </Badge>
                                  )
                                })}
                              </div>
                          ...
                          {/* Services détaillés */}
                          {scenario.input.services && scenario.input.services.length > 0 && (
                            <div className="bg-gradient-to-br from-blue-50 to-indigo-50 rounded-lg p-4 border border-blue-200">
                              <h5 className="font-semibold text-blue-800 mb-3 flex items-center gap-2">
                                <div className="h-1 w-1 bg-blue-500 rounded-full"></div>
                                Services Sélectionnés ({scenario.input.services.length})
                              </h5>
                              <div className="flex flex-wrap gap-2">
                                {scenario.input.services.map(serviceId => {
                                  const service = getServiceData(serviceId, scenario.serviceType)
                                  const name = getServiceName(serviceId, scenario.serviceType)
                                  return (
                                    <Badge key={serviceId} variant="outline" className="bg-white border-blue-300 text-blue-800 text-xs">
                                      {service?.icon && <span className="mr-1">{service.icon}</span>}
                                      {name}
                                      {service?.value && <span className="ml-1 font-bold">+{service.value}€</span>}
                                    </Badge>
                                  )
                                })}
                              </div>
        ...
        <TabsContent value="results">
          <Card className="border-slate-200 shadow-sm bg-white">
            <CardHeader className="bg-gradient-to-r from-slate-50 to-purple-50 border-b border-slate-200">
              ...
              <CardDescription className="text-slate-600">Détails des performances et recommandations</CardDescription>
            </CardHeader>
              ...
              <div className="space-y-4">
                {scenarios.filter(s => s.lastResult).length === 0 ? (
                  <div className="text-center py-12 text-slate-500">
                    ...
                    <p className="text-sm">Lancez des tests pour voir les résultats ici</p>
                  </div>
                ) : (
                  scenarios.filter(s => s.lastResult).map(scenario => (
                    <div key={scenario.id} className="border border-slate-200 rounded-lg p-5 bg-gradient-to-br from-white to-slate-50 shadow-sm">
                      <div className="flex items-center justify-between mb-4">
                        ...
                        >
                          {scenario.lastResult?.recommendation === 'accept' ? '✅ Accepté' :
                           scenario.lastResult?.recommendation === 'review' ? '⚠️ À réviser' : '❌ Rejeté'}
                        </Badge>
                      </div>
                        ...
                        <div className="space-y-4">
                          ...
                          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                            <div className="bg-blue-50 rounded-lg p-3 border border-blue-200">
                              <span className="text-xs text-blue-700 font-medium block mb-1">Prix calculé</span>
                              <div className="text-lg font-bold text-blue-900">{scenario.lastResult.calculatedPrice}€</div>
                            </div>
                            <div className="bg-purple-50 rounded-lg p-3 border border-purple-200">
                              <span className="text-xs text-purple-700 font-medium block mb-1">Prix attendu</span>
                              <div className="text-lg font-bold text-purple-900">{scenario.expectedPrice || 0}€</div>
                            </div>
                            <div className="bg-orange-50 rounded-lg p-3 border border-orange-200">
                              <span className="text-xs text-orange-700 font-medium block mb-1">Écart</span>
                              <div className={`text-lg font-bold ${
                                !scenario.expectedPrice
                                  ? 'text-slate-500'
                                  : scenario.lastResult.variancePercentage <= scenario.tolerance
                                  ? 'text-green-700'
                                  : 'text-red-700'
                              }`}>
                                {!scenario.expectedPrice || isNaN(scenario.lastResult.variancePercentage)
                                  ? 'N/A'
                                  : `${scenario.lastResult.variancePercentage.toFixed(1)}%`}
                              </div>
                            </div>
                            <div className="bg-slate-50 rounded-lg p-3 border border-slate-200">
                              <span className="text-xs text-slate-700 font-medium block mb-1">Temps d'exécution</span>
                              <div className="text-lg font-bold text-slate-900">{scenario.lastResult.executionTime}ms</div>
                            </div>
                          </div>
                            ...
                            <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-sm">
                              {scenario.input.volume !== undefined && (
                                <div className="flex items-center gap-2">
                                  <span className="text-slate-600">Volume:</span>
                                  <Badge variant="outline" className="text-xs">{scenario.input.volume} m³</Badge>
                                </div>
                              )}
                              {scenario.input.distance !== undefined && (
                                <div className="flex items-center gap-2">
                                  <span className="text-slate-600">Distance:</span>
                                  <Badge variant="outline" className="text-xs">{scenario.input.distance} km</Badge>
                                </div>
                              )}
                              {scenario.input.workers !== undefined && (
                                <div className="flex items-center gap-2">
                                  <span className="text-slate-600">Ouvriers:</span>
                                  <Badge variant="outline" className="text-xs">{scenario.input.workers}</Badge>
                                </div>
                              )}
                              {scenario.input.duration !== undefined && (
                                <div className="flex items-center gap-2">
                                  <span className="text-slate-600">Durée:</span>
                                  <Badge variant="outline" className="text-xs">{scenario.input.duration}h</Badge>
                                </div>
                              )}
                            </div>
                          </div>
                          ...
                          {(scenario.input.constraints?.length > 0 || scenario.input.services?.length > 0) && (
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                              {/* Contraintes */}
                              {scenario.input.constraints && scenario.input.constraints.length > 0 && (
                                <div className="bg-gradient-to-br from-orange-50 to-red-50 rounded-lg p-4 border border-orange-200">
                                  <h5 className="font-semibold text-orange-800 mb-3 flex items-center gap-2">
                                    <div className="h-1 w-1 bg-orange-500 rounded-full"></div>
                                    Contraintes Testées ({scenario.input.constraints.length})
                                  </h5>
                                  <div className="flex flex-wrap gap-2">
                                    {scenario.input.constraints.slice(0, 6).map(constraintId => {
                                      const constraint = getConstraintData(constraintId, scenario.serviceType)
                                      const name = getConstraintName(constraintId, scenario.serviceType)
                                      return (
                                        <Badge key={constraintId} variant="outline" className="bg-white border-orange-300 text-orange-800 text-xs">
                              ...
                              {scenario.input.services && scenario.input.services.length > 0 && (
                                <div className="bg-gradient-to-br from-blue-50 to-indigo-50 rounded-lg p-4 border border-blue-200">
                                  <h5 className="font-semibold text-blue-800 mb-3 flex items-center gap-2">
                                    <div className="h-1 w-1 bg-blue-500 rounded-full"></div>
                                    Services Testés ({scenario.input.services.length})
                                  </h5>
                                  <div className="flex flex-wrap gap-2">
                                    {scenario.input.services.slice(0, 6).map(serviceId => {
                                      const service = getServiceData(serviceId, scenario.serviceType)
                                      const name = getServiceName(serviceId, scenario.serviceType)
                                      return (
                                        <Badge key={serviceId} variant="outline" className="bg-white border-blue-300 text-blue-800 text-xs">
                                    ...
                                    {scenario.input.services.length > 6 && (
                                      <Badge variant="outline" className="text-xs bg-white border-blue-300 text-blue-700">
                                        +{scenario.input.services.length - 6} autres
                                      </Badge>
                                    )}
                                  </div>
                                </div>
                          ...
                          {scenario.lastResult.appliedRules && scenario.lastResult.appliedRules.length > 0 && (
                            <div className="bg-gradient-to-br from-green-50 to-emerald-50 rounded-lg p-4 border border-green-200">
                              <h5 className="font-semibold text-green-800 mb-3 flex items-center gap-2">
                                <div className="h-1 w-1 bg-green-500 rounded-full"></div>
                                Règles Appliquées par l'API ({scenario.lastResult.appliedRules.length})
                              </h5>
                              <div className="flex flex-wrap gap-2">
                                {scenario.lastResult.appliedRules.map((rule, index) => (
                                  <Badge
                                    key={index}
                                    variant="outline"
                                  ...
                                  >
                                    {rule.name}: {rule.impact > 0 ? '+' : ''}{rule.impact}€
                                  </Badge>
                                ))}
                              </div>
                            </div>
                          ...
                          <div className={`rounded-lg p-4 border ${
                            ...
                            <h5 className={`font-semibold mb-2 ${
                              ...
                              {!scenario.expectedPrice && 'ℹ️ Test sans prix de référence'}
                              {scenario.expectedPrice && scenario.lastResult.recommendation === 'accept' && '✅ Test Validé'}
                              {scenario.expectedPrice && scenario.lastResult.recommendation === 'review' && '⚠️ Test à Réviser'}
                              {scenario.expectedPrice && scenario.lastResult.recommendation === 'reject' && '❌ Test Rejeté'}
                            </h5>
                            <p className="text-sm text-slate-700">
                              ...
                              {scenario.expectedPrice && scenario.lastResult.recommendation === 'accept' &&
                                `L'écart de ${scenario.lastResult.variancePercentage.toFixed(1)}% est acceptable (tolérance: ${scenario.tolerance}%).`}
                              {scenario.expectedPrice && scenario.lastResult.recommendation === 'review' &&
                              ...
                              {scenario.expectedPrice && scenario.lastResult.recommendation === 'reject' &&
