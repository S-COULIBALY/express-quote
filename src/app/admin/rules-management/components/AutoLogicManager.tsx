...
export function AutoLogicManager() {
    ...
    const rules: AutoLogicRule[] = [
      {
        ...
        description:
          "Détermine automatiquement si un monte-meuble est nécessaire (via AutoDetectionService)",
      ...
      },
      {
        ...
        isActive: true,
        parameters: {
          surchargeAmount: DefaultValues.SERVICE_WORKER_PRICE_PER_HOUR, // ✅ Corrigé (anciennement WORKER_HOUR_RATE)
      ...
      },
      {
        ...
        description:
          "Détecte et ajoute automatiquement la contrainte de portage longue (via AutoDetectionService)",
  ...
  const runSingleTest = async (ruleId: string, scenarioId: string) => {
    ...
    try {
      ...
      switch (rule.algorithm) {
        ...
        case "detectFurnitureLift": {
          const addressData: AddressData = {
            ...
            elevator: scenario.input.elevator as
              | "no"
              | "small"
              | "medium"
              | "large",
          ...
          const detectionResult =
            AutoDetectionService.detectFurnitureLift(addressData);
        ...
        case "analyzeCarryingDistance": {
          const pickupData: AddressData = {
            floor: 0,
            elevator: "no",
            carryDistance: scenario.input.pickupCarryDistance as
              | "0-10"
              | "10-30"
              | "30+"
              | undefined,
          ...
          const deliveryData: AddressData = {
            floor: 0,
            elevator: "no",
            carryDistance: scenario.input.deliveryCarryDistance as
              | "0-10"
              | "10-30"
              | "30+"
              | undefined,
          ...
          const pickupResult =
            AutoDetectionService.detectLongCarryingDistance(pickupData);
          const deliveryResult =
            AutoDetectionService.detectLongCarryingDistance(deliveryData);

          result = {
            pickupDistanceInMeters:
              pickupData.carryDistance === "30+"
                ? 35
                : pickupData.carryDistance === "10-30"
                  ? 20
                  : 5,
            deliveryDistanceInMeters:
              deliveryData.carryDistance === "30+"
                ? 35
                : deliveryData.carryDistance === "10-30"
                  ? 20
                  : 5,
            requiresLongCarryingConstraint:
              pickupResult.longCarryingDistance ||
              deliveryResult.longCarryingDistance,
            affectedAddresses: [
      ...
      const passed =
        JSON.stringify(result) === JSON.stringify(scenario.expectedOutput);
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            ...
            <Badge variant="outline" className="text-xs">
              {logicRules.filter((r) => r.isActive).length} /{" "}
              {logicRules.length}
            </Badge>
          </div>
          {logicRules.map((rule) => {
            const hasFailedTests = rule.testScenarios.some(
              (s) => s.passed === false,
            );
            const allTestsPassed = rule.testScenarios.every(
              (s) => s.passed === true,
            );
              ...
              <CardContent className="space-y-6 pt-6">
                ...
                <div className="bg-slate-50 rounded-lg p-4 border border-slate-200">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {Object.entries(selectedRuleData.parameters).map(
                      ([key, value]) => (
                  ...
                  <div className="space-y-3">
                    {selectedRuleData.testScenarios.map((scenario) => {
                      ...
                      return (
                        <div
                          key={scenario.id}
                          className="bg-white border border-green-200 rounded-lg p-4 shadow-sm transition-all duration-200 hover:shadow-md"
                        >
                          <div className="flex items-center justify-between mb-3">
                            ...
                            <div className="flex items-center gap-2">
                              {scenario.passed !== undefined && (
                                <Badge
                                  variant={
                                    scenario.passed ? "default" : "destructive"
                                  }
                          ...
                          <div className="space-y-2 text-xs">
                            ...
                            <div className="flex items-start gap-2">
                              <span className="font-semibold text-purple-700 min-w-[60px]">
                                Attendu:
                              </span>
                            ...
                            {scenario.lastResult !== undefined && (
                              <div className="flex items-start gap-2">
                                ...
                                <code
                                  className={`flex-1 px-2 py-1 rounded border font-mono ${
                                    scenario.passed
