...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    try {
      ...
      const professionalResponseRate =
        await calculateProfessionalResponseRate(prisma);
      ...
      const response: AttributionStatsResponse = {
        ...
        professionalSessions: professionalSessions.map((session: any) => ({
          ...
          professionalName:
            session.professional?.companyName || "Professionnel",
        ...
        recentUpdates: recentUpdates.map((update: any) => ({
          ...
          deliveredCount:
            update.deliveredCount || (update.acknowledged ? 1 : 0),
...
async function calculateAverageResponseTime(prisma: any): Promise<number> {
  try {
    ...
    const totalMinutes = recentAttributions.reduce((sum: any, attr: any) => {
      const diff =
        new Date(attr.updatedAt).getTime() - new Date(attr.createdAt).getTime();
...
async function handleForceUpdate(
  ...
  try {
    ...
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_APP_URL}/api/attribution/broadcast-update`,
      {
        method: "POST",
...
async function handleCleanupOldData(logger: any): Promise<NextResponse> {
  try {
    ...
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_APP_URL}/api/attribution/updates`,
      {
        method: "DELETE",
      },
    );
    ...
    return {
      rate:
        total_attributions > 0
          ? Math.round((accepted_attributions / total_attributions) * 100)
          : 0,
...
async function calculateBusinessMetrics(prisma: any) {
  try {
    ...
    const totalValue = activeAttributions.reduce(
      (sum: any, attr: any) => sum + (attr.booking?.totalAmount || 0),
      0,
    );

    const lostValue = lostAttributions.reduce(
      (sum: any, attr: any) => sum + (attr.booking?.totalAmount || 0),
      0,
    );

    const completedValue = completedAttributions.reduce(
      (sum: any, attr: any) => sum + (attr.booking?.totalAmount || 0),
      0,
    );

    const averageOrderValue =
      completedAttributions.length > 0
    ...
    const roi =
      completedValue > 0
        ? Math.round(
            85 +
              (completedAttributions.length /
                (activeAttributions.length +
                  completedAttributions.length +
                  lostAttributions.length)) *
                100 *
                0.3,
          )
        : 0;
...
async function calculateRebroadcastRate(prisma: any) {
  try {
    ...
    const rebroadcastAttributions = await prisma.booking_attributions.count({
      where: {
        OR: [{ status: "RE_BROADCASTING" }, { broadcastCount: { gt: 1 } }],
      },
    });
    ...
    return {
      ...
      percentage:
        total_attributions > 0
          ? Math.round((rebroadcastAttributions / total_attributions) * 100)
          : 0,
    };
  } catch (error) {
...
async function calculateProfessionalResponseRate(prisma: any) {
  try {
    ...
    return totalEligible > 0
      ? Math.round((responded / totalEligible) * 100)
      : 0;
  } catch (error) {
    return 0;
  }
