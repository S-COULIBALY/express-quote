...
export async function GET(request: NextRequest) {
  try {
    logger.info(
      "🔍 GET /api/bookings/current - Récupération réservation en cours",
    );
    ...
    try {
      ...
      if (!booking) {
        logger.info(
          `Réservation ${bookingId} non trouvée dans la base de données`,
        );
        return NextResponse.json({
          success: true,
          data: { cart: { items: [] } },
    ...
    logger.error(
      "Erreur lors de la récupération de la réservation en cours:",
      error,
    );
    return NextResponse.json(
      {
