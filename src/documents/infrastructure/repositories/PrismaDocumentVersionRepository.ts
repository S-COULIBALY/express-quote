...
import { DocumentVersion, VersionStatus } from '../../domain/entities/DocumentVersion';
import { IDocumentVersionRepository, VersionSearchCriteria, VersionStatistics } from '../../domain/repositories/IDocumentVersionRepository';
...
export class PrismaDocumentVersionRepository implements IDocumentVersionRepository {
  ...
  async save(version: DocumentVersion): Promise<DocumentVersion> {
    try {
      ...
      logger.error('❌ Erreur lors de la sauvegarde de la version', error as Error);
      throw error;
    }
  }
  ...
  async findById(id: string): Promise<DocumentVersion | null> {
    try {
      ...
      logger.error('❌ Erreur lors de la recherche de version par ID', error as Error);
      throw error;
    }
  }
  ...
  async findByDocumentId(documentId: string): Promise<DocumentVersion[]> {
    try {
      ...
      logger.error('❌ Erreur lors de la recherche de versions par document', error as Error);
      throw error;
    }
  }

  async findCurrentPublishedVersion(documentId: string): Promise<DocumentVersion | null> {
    try {
      ...
      logger.error('❌ Erreur lors de la recherche de version publiée', error as Error);
      throw error;
    }
  }

  async findByCriteria(criteria: VersionSearchCriteria): Promise<DocumentVersion[]> {
    try {
      const where: any = {};
      ...
      logger.error('❌ Erreur lors de la recherche de versions par critères', error as Error);
      throw error;
    }
  }

  async findPendingApprovalsForUser(userId: string, userRole: string): Promise<DocumentVersion[]> {
    try {
      ...
      const mappedVersions = versions.map(version => this.mapPrismaToVersion(version));
      ...
      logger.error('❌ Erreur lors de la recherche d\'approbations en attente', error as Error);
      throw error;
    }
  }
  ...
  async update(version: DocumentVersion): Promise<DocumentVersion> {
    try {
      ...
      logger.error('❌ Erreur lors de la mise à jour de version', error as Error);
      throw error;
    }
  }
  ...
  async delete(id: string): Promise<boolean> {
    try {
      ...
      logger.error('❌ Erreur lors de la suppression de version', error as Error);
      return false;
    }
  }
  ...
  async getStatistics(dateRange?: { start: Date; end: Date }): Promise<VersionStatistics> {
    try {
      const where: any = {};
      ...
      // Compter par statut
      const byStatusPromises = Object.values(VersionStatus).map(async status => {
      ...
      const pendingApprovals = (byStatus[VersionStatus.PENDING_REVIEW] || 0) + 
      ...
      if (approvedVersions.length > 0) {
        const totalApprovalTime = approvedVersions.reduce((sum, version) => {
          const diffMs = version.approvedAt!.getTime() - version.createdAt.getTime();
          return sum + (diffMs / (1000 * 60 * 60)); // Convertir en heures
        }, 0);
        
        averageApprovalTime = Math.round(totalApprovalTime / approvedVersions.length);
      }

      // Calculer le taux de rejet
      const rejectedCount = byStatus[VersionStatus.REJECTED] || 0;
      const rejectionRate = totalVersions > 0 ? 
        Math.round((rejectedCount / totalVersions) * 100) : 0;
  ...
  async archiveOldVersions(documentId: string, keepCount: number = 5): Promise<number> {
    try {
      ...
      logger.error('❌ Erreur lors de l\'archivage des anciennes versions', error as Error);
      throw error;
    }
  }
  ...
  async findExpiredVersions(): Promise<DocumentVersion[]> {
    try {
      ...
      logger.error('❌ Erreur lors de la recherche de versions expirées', error as Error);
      throw error;
    }
  }
    ...
    try {
      ...
      const activeVersions = await this.prisma.documentVersion.findMany({
        where: {
          documentId,
          status: {
            in: [VersionStatus.DRAFT, VersionStatus.PENDING_REVIEW, VersionStatus.UNDER_REVIEW]
          }
        },
        orderBy: { createdAt: 'desc' }
      });

      const conflictingVersions = activeVersions.map(version => this.mapPrismaToVersion(version));
      ...
      logger.error('❌ Erreur lors de la recherche de conflits', error as Error);
      throw error;
    }
  }

  async cloneVersion(versionId: string, newVersionNumber: string, createdBy: string): Promise<DocumentVersion> {
    try {
  ...
  async rollbackToVersion(documentId: string, targetVersionId: string, rollbackBy: string): Promise<DocumentVersion> {
    try {
      ...
      const nextVersionNumber = this.generateNextVersionNumber(existingVersions);
   ...
   */
  private generateNextVersionNumber(existingVersions: DocumentVersion[]): string {
