...
class AbandonAnalytics {
  ...
  async recordAbandonEvent(event: AbandonEvent): Promise<void> {
    try {
      ...
      logger.error(
        "Erreur lors de l'enregistrement d'événement d'abandon:",
        error,
      );
    }
  }
  ...
  async recordRecoveryEvent(
    ...
    try {
      ...
      logger.error(
        "Erreur lors de l'enregistrement d'événement de récupération:",
        error,
      );
    }
  }
  ...
  generateAbandonMetrics(dateFrom?: Date, dateTo?: Date): AbandonMetrics {
    try {
      ...
      const recoveredAbandons = filteredEvents.filter(
        (e) => e.isRecovered,
      ).length;
      const recoveryRate =
        totalAbandons > 0 ? (recoveredAbandons / totalAbandons) * 100 : 0;
      ...
      const averageTimeToAbandon =
        this.calculateAverageTimeToAbandon(filteredEvents);

      // Conversion après récupération
      const conversionAfterRecovery =
        this.calculateConversionAfterRecovery(filteredEvents);

      // Revenus perdus et récupérés
      const { lossRevenue, recoveredRevenue } =
        this.calculateRevenueMetrics(filteredEvents);
      ...
      logger.error(
        "Erreur lors de la génération des métriques d'abandon:",
        error,
      );
      throw error;
    }
  ...
  generateRecoveryMetrics(dateFrom?: Date, dateTo?: Date): RecoveryMetrics {
    try {
      const filteredRecoveries = this.filterRecoveryEventsByDate(
        dateFrom,
        dateTo,
      );

      const totalRecoveryAttempts = filteredRecoveries.length;
      const successfulRecoveries = filteredRecoveries.filter(
        (r) => r.success,
      ).length;
      ...
      const avgTimeToRecovery =
        this.calculateAverageTimeToRecovery(filteredRecoveries);

      // Récupération par incentive
      const recoveryByIncentive =
        this.groupRecoveryByIncentive(filteredRecoveries);

      // Coût par récupération
      const totalCost = filteredRecoveries.reduce((sum, r) => sum + r.cost, 0);
      const costPerRecovery =
        totalRecoveryAttempts > 0 ? totalCost / totalRecoveryAttempts : 0;
      ...
      logger.error(
        "Erreur lors de la génération des métriques de récupération:",
        error,
      );
      throw error;
    }
  ...
  generateCustomerSegmentMetrics(): CustomerSegmentMetrics[] {
    try {
      ...
      for (const segment of segments) {
        const segmentEvents = this.events.filter(
          (e) => this.getCustomerSegment(e) === segment,
        );
        const segmentRecoveries = this.recoveryEvents.filter((r) =>
          segmentEvents.some((e) => e.id === r.eventId),
        );

        const totalCustomers = new Set(segmentEvents.map((e) => e.sessionId))
          .size;
        const abandonRate = this.calculateSegmentAbandonRate(segmentEvents);
        const recoveryRate = this.calculateSegmentRecoveryRate(
          segmentEvents,
          segmentRecoveries,
        );
        const avgOrderValue = this.calculateSegmentAOV(segmentEvents);
        const lifetimeValue = this.calculateSegmentLTV(segmentEvents);
        const mostCommonAbandonStage =
          this.getMostCommonAbandonStage(segmentEvents);
        const bestRecoveryChannel =
          this.getBestRecoveryChannel(segmentRecoveries);
      ...
      logger.error(
        "Erreur lors de la génération des métriques de segments:",
        error,
      );
      return [];
    }
  ...
  private async realTimeAnalysis(event: AbandonEvent): Promise<void> {
    try {
      ...
      const recentEvents = this.events.filter(
        (e) => e.timestamp > new Date(Date.now() - 60 * 60 * 1000), // Dernière heure
      );
  ...
  private detectTechnicalIssues(events: AbandonEvent[]): string[] {
    ...
    const earlyAbandons = events.filter(
      (e) => e.stage === "catalog_early",
    ).length;
    ...
    const paymentAbandons = events.filter(
      (e) => e.stage === "payment_page",
    ).length;
    ...
    const repeatedAbandons = Array.from(sessionCounts.values()).filter(
      (count) => count > 3,
    ).length;
  ...
  private filterRecoveryEventsByDate(
    dateFrom?: Date,
    dateTo?: Date,
  ): typeof this.recoveryEvents {
  ...
  private calculateConversionAfterRecovery(events: AbandonEvent[]): number {
    ...
    const conversions = recoveredEvents.filter(
      (e) => e.recoveryAttempts > 0,
    ).length;
    return Math.round((conversions / recoveredEvents.length) * 100);
  }

  private calculateRevenueMetrics(events: AbandonEvent[]): {
    lossRevenue: number;
    recoveredRevenue: number;
  } {
    let lossRevenue = 0;
  ...
  private analyzeFunnel(events: AbandonEvent[]): FunnelStageMetrics[] {
    ...
    stages.forEach((stage, index) => {
      ...
      const avgTimeSpent =
        entries > 0
          ? stageEvents.reduce((sum, e) => sum + e.timeSpent, 0) / entries
          : 0;
      ...
      const nextStageEvents = nextStage
        ? events.filter((e) => e.stage === nextStage).length
        : 0;
      const dropoffRate =
        entries > 0
          ? Math.max(0, ((entries - nextStageEvents) / entries) * 100)
          : 0;
  ...
  private groupRecoveryByChannel(
    recoveries: typeof this.recoveryEvents,
  ...
  private groupRecoveryByStage(
    recoveries: typeof this.recoveryEvents,
  ): Record<AbandonStage, number> {
    const grouped = {} as Record<AbandonStage, number>;
  ...
  private calculateAverageTimeToRecovery(
    recoveries: typeof this.recoveryEvents,
  ): number {
    ...
    recoveries.forEach((recovery) => {
      ...
      if (originalEvent) {
        totalTime +=
          recovery.timestamp.getTime() - originalEvent.timestamp.getTime();
        count++;
      }
    });
  ...
  private groupRecoveryByIncentive(
    recoveries: typeof this.recoveryEvents,
  ): Record<string, number> {
    const grouped: Record<string, number> = {};
  ...
  private calculateRecoveryROI(recoveries: typeof this.recoveryEvents): number {
    ...
    return totalCost > 0
      ? ((recoveredRevenue - totalCost) / totalCost) * 100
      : 0;
  }
  ...
  private getMostCommonAbandonStage(events: AbandonEvent[]): AbandonStage {
    ...
    const sortedStages = Object.entries(stageCounts).sort(
      ([, a], [, b]) => b - a,
    );
    return (sortedStages[0]?.[0] as AbandonStage) || "catalog_early";
  }

  private getBestRecoveryChannel(
    recoveries: typeof this.recoveryEvents,
  ): string {
  ...
  private cleanupOldEvents(): void {
    ...
    this.recoveryEvents = this.recoveryEvents.filter(
...
export const useAbandonAnalytics = () => {
  ...
  return {
    ...
    getMetrics: (dateFrom?: Date, dateTo?: Date) =>
      analytics.generateAbandonMetrics(dateFrom, dateTo),
    getRecoveryMetrics: (dateFrom?: Date, dateTo?: Date) =>
      analytics.generateRecoveryMetrics(dateFrom, dateTo),
